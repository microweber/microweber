<script>
    AddToCartModalContent = window.AddToCartModalContent || function (title) {
        var html = ''
            ...
            + '<h5>' + title + '</h5>'
            + '<p>{{ _ejs("has been added to your cart") }}</p><br />'
            + '<div><a href="javascript:;" onclick="mw.tools.modal.remove(\'#AddToCartModal\')" class="pull-left mt-2">{{ _e("Continue shopping") }}</a>'
            + '<a href="{{ checkout_url() }}" class="btn-d pull-right">{{ _ejs("Checkout") }}</a></section><div class="clearfix"></div></div>';
            ...
            if (step == 'payment-method') {
                $('.js-delivery-address input').each(function () {
                    if (!this.checkValidity()) {
                        mw.notification.warning('{{ _e("Please fill the required fields") }}');
                        $(this).addClass('error');
                        has_error = 1;
                    } else {
