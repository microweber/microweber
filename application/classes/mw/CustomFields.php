...
// $data_to_save['debug'] = 1;
  ...
  static function make_default($rel, $rel_id, $fields_csv_str)
    {
        ...
        if (isset($rel)) {
            ...
            if (isarr($fields_csv_str)) {
                foreach ($fields_csv_str as $field_type) {
                    $ex = self::get($rel, $rel_id, $return_full = 1, $field_for = false, $debug = 0, $field_type);
   ...
   static function delete($id)
    ...
    static function make_field($field_id = 0, $field_type = 'text', $settings = false) {
        $data = false;
        $form_data = array();
        if (is_array($field_id)) {
            ...
            if (!empty($field_id)) {
                $data = $field_id;
                return self::make($field_id, false, 'y');
            }
        } else {
            if ($field_id != 0) {

                return self::make($field_id);
     ...
     */
    static function make($field_id = 0, $field_type = 'text', $settings = false)
    {
