...
class Cart_model extends CI_Model {
	...
	function __construct() {
		...
		if (strval ( trim ( $country_name ) ) == '') {
			...
			$this->session->set_userdata ( 'country_name', USER_COUNTRY_NAME );
		...
		$shop_currency = $this->session->userdata ( 'shop_currency' );
	...
	function itemAdd($data) {
		...
		if (($data ['height']) == false) {
			$data ['height'] = floatval ( cf_val ( $post ['id'], 'height' ) );
		}
		if (($data ['width']) == false) {
			$data ['width'] = floatval ( cf_val ( $post ['id'], 'width' ) );
		}
		if (($data ['length']) == false) {
			$data ['length'] = floatval ( cf_val ( $post ['id'], 'length' ) );
		}
		if (($data ['weight']) == false) {
			$data ['weight'] = floatval ( cf_val ( $post ['id'], 'weight' ) );
		}
		...
		if (($data ['price']) == false) {
			$data ['price'] = floatval ( cf_val ( $post ['id'], 'price' ) );
		}
		...
		if (($data ['item_name']) == false) {
			if ($data ['to_table'] == 'table_content' and $data ['to_table_id'] != false) {
				$content_data = $this->content_model->contentGetById ( $data ['to_table_id'] );
	...
	function itemsGet($data = false, $limit = false, $offset = false, $orderby = false, $cache_group = false, $debug = false, $ids = false, $count_only = false, $only_those_fields = false, $exclude_ids = false, $force_cache_id = false, $get_only_whats_requested_without_additional_stuff = false) {
		...
		if (! empty ( $get )) {
			foreach ( $get as $g ) {
				$more = false;
				$more = $this->core_model->getCustomFields ( 'table_cart', $g ['id'], true );
	...
	function billing_borica_ProcessCreditCard() {
		...
		$formdata ['shipping_total_charges'] = $this->session->userdata ( 'shipping' );
		$formdata ['amount'] = floatval ( $this->itemsGetTotal ( $this->session->userdata ( 'cart_promo_code' ), $this->session->userdata ( 'shop_currency' ) ) );
		$formdata ['amount'] = $formdata ['amount'] + $formdata ['shipping_total_charges'];
		$formdata ['first_name'] = $this->session->userdata ( 'first_name' );
		$formdata ['last_name'] = $this->session->userdata ( 'last_name' );
		$formdata ['country'] = $this->session->userdata ( 'country' );
		$formdata ['phone'] = $this->session->userdata ( 'night_phone_a' );
		$formdata ['email'] = $this->session->userdata ( 'email' );
		//	$formdata ['country'] = CI::library('session')->userdata ( 'country' );
		$formdata ['city'] = $this->session->userdata ( 'city' );
		
		$formdata ['state'] = $this->session->userdata ( 'state' );
		$formdata ['zip'] = $this->session->userdata ( 'zip' );
		...
		$formdata ['amount'] = $this->currencyExchange ( $from = $this->session->userdata ( 'shop_currency' ), $to = "bgn", $formdata ['amount'] );
	...
	function billingProcessCreditCard($make_transaction = false) {
		...
		$formdata ['amount'] = floatval ( $this->itemsGetTotal ( $this->session->userdata ( 'cart_promo_code' ), $this->session->userdata ( 'shop_currency' ) ) ) + ( float ) (rand ( 1, 20 ) / 100);
		...
		if (function_exists ( "curl_init" )) {
			...
			$userData ['first_name'] = $this->session->userdata ( 'billing_first_name' );
			$userData ['last_name'] = $this->session->userdata ( 'billing_last_name' );
			$userData ['credit_card_type'] = $this->session->userdata ( 'credit_card_type' );
			...
			$userData ['cvv2'] = $this->session->userdata ( 'billing_cvv2' );
			...
			if ("SUCCESS" == strtoupper ( $httpParsedResponseAr ["ACK"] ) || "SUCCESSWITHWARNING" == strtoupper ( $httpParsedResponseAr ["ACK"] )) {
				...
				if (isset ( $formdata ['transactionid'] )) {
					...
					$this->session->set_userdata ( 'order_id', '' . date ( 'ymdHis' ) . rand () );
	...
	function orderConfirm($sid = false, $order_id = false) {
		...
		$this->session->set_userdata ( 'cart_promo_code', false );
	...
	function orderPlace($data) {
		...
		$get_option1 = $this->core_model->optionsGetByKey ( $get_option, true );
		$subj = $get_option1 ['option_value'];
		
		if (trim ( $subj ) == '') {
			
			$this->core_model->optionsSetDefault ( 'order_complete_email' );
	...
	function promoCodeGetCurent() {
		...
		$code = $this->session->userdata ( 'promo_code' );
		$code = trim ( $code );
		if ($code != '') {
	...
	function ups_shiping_cost($ship_to, $ship_from, $shipment) {
		/////////////////////////////////////////////////////////////////////////////////////////////
		...
		$access_license_number = $this->core_model->optionsGetByKey ( 'shop_ups_access_license_number' );
		
		$user_id = $this->core_model->optionsGetByKey ( 'shop_ups_username' );
		$password = $this->core_model->optionsGetByKey ( 'shop_ups_password' );
		$shipper_number = $this->core_model->optionsGetByKey ( 'shop_ups_shipper_number' );
		
		$shipper_postal_code = $this->core_model->optionsGetByKey ( 'shop_ups_shipper_postal_code' );
		
		$shipment_service_code = $this->core_model->optionsGetByKey ( 'shop_ups_shipment_service_code' );
		$shipment_to_zip = '20748';
		$upsRate = new upsRate ();
		...
		foreach ( $cart_items as $cart_item ) {
			$shipment = array ();
			$shipment ['length'] = ceil($cart_item ['length']);
			$shipment ['width'] =  ceil($cart_item ['width']);
			$shipment ['height'] = ceil( $cart_item ['height']);
			$shipment ['weight'] =  ceil($cart_item ['weight']);
			//p($shipment);  
			$rate = $upsRate->getRate ( $shipper_postal_code, $shipment_to_zip, $shipment_service_code, $shipment ['length'], $shipment ['width'], $shipment ['height'], $shipment ['weight'] );
			
			//var_dump ( $cart_item );
			$rate1 = $rate * $cart_item ['qty'];
			//..	var_dump ( $rate1 );
			$total_rate = $total_rate + $rate1;
		}
