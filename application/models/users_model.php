...
class Users_model extends CI_Model {
	...
	function saveUser($data) {
		...
		if (intval($data['id']) == 0) {
			$accounts_expiration = $this -> core_model -> optionsGetByKey('accounts_expiration');
		...
		if (intval($data['parent_id']) != 0) {
			$this -> core_model -> cleanCacheGroup('users/' . $data['parent_id']);
		}
	...
	function getUsers($data = false, $limit = false, $count_only = false) {
		...
		$cache_content =  get_instance() -> core_model -> cacheGetContentAndDecode($function_cache_id, $cache_group);
		...
		if (($cache_content) != false) {
			...
			$get =  get_instance() -> core_model -> getDbData($table, $criteria = $data, $limit, $offset = false, $orderby = array('updated_on', 'DESC'), $cache_group, $debug = false, $ids, $count_only = $count_only, $only_those_fields, $exclude_ids = false, $force_cache_id = false, $get_only_whats_requested_without_additional_stuff = true);
			//var_dump($get, $function_cache_id, $cache_group);
			get_instance() -> core_model -> cacheWriteAndEncode($get, $function_cache_id, $cache_group);
	...
	function userDeleteById($id) {
		...
		$data = array();
		$data['user_id'] = $id;
		$this -> core_model -> deleteData($table, $data, $delete_cache_group = false);
		...
		$data = array();
		$data['created_by'] = $id;
		$this -> core_model -> deleteData($log_table, $data, $delete_cache_group = false);
	...
	function forgot_pass($user_email) {

		$u = $this -> users_model -> getUsers($data = array('email' => $user_email), $limit = false, $count_only = false);
		$opt = $u[0];
		if (empty($opt)) {
			return false;
		}
		...
		$from = (empty($admin_options)) ? 'noreply@microweber.com' : $admin_options['option_value'];
		$opt['name'] = user_name($opt['id']);
		$object = $opt['object'];
		if (!$object)
...
STR;
	...
	function send_activation($user_email) {
		...
		$activation_url = site_url('api/user/activate/code:' . md5($opt['id']));
		$to = $opt['email'];
		$opt['site'] = site_url();
		$admin_options = $this -> core_model -> optionsGetByKey('admin_email', true);
		...
		$object = $opt['object'];
		if (!$object)
			$object = 'Email verification link';
		$total = 0;
		$message = <<<STR
		Hello, <b>{name}</b>!
		
		Please click here to activate your account  {activation_url}

		Here You are login details from site <b>{site}</b>:<br />
...
STR;
		...
		$m2 = $this -> core_model -> optionsGetByKey('activation_email', true);
		if ($m2["option_value"]) {
			$message = $m2["option_value"];
		}

		$message = str_ireplace("{activation_url}", $activation_url, $message);
		$message = str_ireplace("{username}", $opt['username'], $message);
		$message = str_ireplace("{user}", $opt['username'], $message);

		$message = str_ireplace("{link}", $activation_url, $message);

		$message = str_ireplace("{password}", $opt['password'], $message);
		$message = str_ireplace("{pass}", $opt['password'], $message);

		$message = str_ireplace("{name}", $opt['name'], $message);
		$message = str_ireplace("{site}", $opt['site'], $message);
		$message = str_ireplace("{site_url}", $opt['site'], $message);
...
STR;
	...
	public function saveFollower($aData) {
		...
		if ($follow) {
			...
			if ($cancel == true) {
				$return = $this -> core_model -> deleteData(TABLE_PREFIX . 'followers', array('user_id' => $followerId, 'follower_id' => $userId));
	...
	public function getFollowers($aUserId = false, $aOnlyIds = false, $db_options = false) {
		...
		$followers = $this -> core_model -> fetchDbData($table, $params, $db_options1);
		//p($followers);
		$return = array();
		...
		$this -> db -> query('REPLACE
                             INTO bb_users
                              SET id              = ?,
                                  ...
                                  user_registered = ?,
                                  user_status     = ?,
                                  display_name    = ?', array($user['id'], $user['username'], $this -> hash_password($user['password']), $user['username'], $user['email'], $user['website'], $user['created_on'], ($user['is_active'] == 'y' ? 0 : 1), $user['username']));
