...
class taxonomy_model extends CI_Model {
	...
	function taxonomyGet($data = false, $orderby = false, $no_limits = false, $no_cache = false) {
		...
		if (intval ( $data ['id'] ) != 0) {
			...
			$save = $this->core_model->getDbData ( $table, $data, $limit = $limit, $offset = false, $orderby, $cache_group = $cache_group, $debug = $data ['debug'], $ids = false, $count_only = false, $only_those_fields = false, $exclude_ids = false, $force_cache_id = false );
	...
	function getUrlForId($id) {
		...
		if (($cache_content) != false) {
			...
			$parent_ids = $this->getParentsIdsAndCache ( $data ['id'] );
		...
		//$url = $url . '/categories:' . $data ['id'];
				...
				if ($url != false) {
					$this->core_model->cacheWriteAndEncode ( $url, $function_cache_id, $cache_group );
					return $url;
	...
	function taxonomyDelete($id) {
		...
		foreach ( $parr as $par ) {
			$this->core_model->cleanCacheGroup ( 'taxonomy' . DIRECTORY_SEPARATOR . $par );
		}
	...
	function getParentsIdsAndCache($id, $without_main_parrent = false, $taxonomy_type = 'category') {
		...
		if (($cache_content) != false) {
			...
			$to_cache = $this->getParentsIds ( $id, $without_main_parrent, $taxonomy_type );
	...
	function get_page_for_category($id) {
		...
		if (($cache_content) != false) {
			...
			$data = $this->taxonomy_model->getSingleItem ( $id );
			...
			$content ['content_subtype_value'] = $data ['id'] ;
			
		 //	p($content);

			$content = $this->content_model->getContentAndCache ( $content );
		...
		//	
			...
			$parent_ids = $this->taxonomy_model->getParentsIdsAndCache ( $data ['id'] );
