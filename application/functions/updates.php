...
function mw_check_for_module_update($module = false) {
	$upd_count = mw_check_for_update();

	if (isset($upd_count['modules']) and isarr($upd_count['modules'])) {
		if ($module != false) {
			foreach ($upd_count['modules'] as $value) {
				if (isset($value['module']) and $value['module'] == $module) {
					return true;
				}
			}
		} else {
			return $upd_count['modules'];
		}
	} else {
		return false;
	}
