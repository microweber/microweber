...
function mw_updates_count()
{
    ...
    $cache_content = cache_get_content('mw_updates_count'.date("ymdh"), 'update/global');
	...
	register_shutdown_function('mw_check_for_update');


   // $scheduler = new \mw\utils\Events();

    // schedule a global scope function:
 //   $scheduler->registerShutdownEvent("mw_check_for_update");
...
function mw_check_for_update()
{
    ...
    if ($mw_avail_updates == false) {
        ...
        if (isset($mw_avail_updates['count'])) {
            //   return intval($upd_count['count']);

            cache_save($mw_avail_updates['count'], 'mw_updates_count'.date("ymdh"), 'update/global');
