   ...
   $.fn.liveDraggable = function (opts) {
      this.live("mouseover", function() {
	   ...
	   this.live("mouseout", function() {
         if (!$(this).data("init")) {
            $(this).data("init", true).draggable('destroy');
         }
      });
      return $();
   };
...
function init_sortables(){
	       // $('#mercury_iframe').contents().find('.edit').html('Hey, i`ve changed content of  body>! Yay!!!');
			...
			//$(".edit").find('div,.module,img,ul,ol,a,table,pre,p,span,h1,h2,h3,h4,h5,h6,h7,h8,blockquote').liveDraggable({
			$(".edit").find('*:not(.ui-draggable)').liveDraggable({
			// appendTo: 'body',
			 stack: "div",
			 //snapMode: 'outer',
