...
class Media extends CI_Controller {
	...
	function upload_to_library() {
		...
		if ($_POST) {
			...
			$status = $this->core_model->upload ( $for, $id, $queue_id, $collection = $module_id );
			//p ( $status );
	...
	function upload_base64() {
		...
		$status = $this->core_model->upload_base64 ( $file, $filename );
	...
	function user_upload_picture() {
		...
		if (! empty ( $_FILES )) {
			...
			$id = $this->core_model->guessId ();
			if ($to_table == '') {
				$to_table = 'table_content';
			}
			
			$res = $this->core_model->mediaUpload ( $to_table, $id, $queue_id = false, $resize_options );
			$res = json_encode ( $res );
			exit ( $res );
		}
