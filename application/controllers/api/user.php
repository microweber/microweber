...
class User extends CI_Controller {
	...
	function loginas() {
		...
		$this->session->set_userdata ( 'user', $other_user );
	...
	function register() {
		...
		if ($_POST) {
			...
			$captcha_seesion = $this->session->userdata ( 'captcha' );
...
and to_table_id='{$message['to_table_id']}'
	...
	function message_read() {
		...
		if ($_POST) {
			...
			if (! empty ( $thread )) {
				$thread_ids = $this->core_model->dbExtractIdsFromArray ( $thread );
				if (! empty ( $thread_ids )) {
	...
	function message_delete() {
		...
		if ($_POST) {
			...
			if (intval ( $message ['parent_id'] ) == 0) {
				...
				$deleted = $this->core_model->saveData ( TABLE_PREFIX . 'messages', array ('id' => $message ['id'], 'deleted_from_sender' => 'y', 'deleted_from_receiver' => 'y', 'is_read' => 'y' ) );
				$cache_group = 'users/messages/' . $message ['parent_id'];
				$this->core_model->cleanCacheGroup ( $cache_group );
			} else {
				$cache_group = 'users/messages/' . $message ['id'];
				$this->core_model->cleanCacheGroup ( $cache_group );
				$deleted = $this->core_model->saveData ( TABLE_PREFIX . 'messages', array ('id' => $message ['id'], 'deleted_from_' . $deletedFrom => 'y', 'is_read' => 'y' ) );
			}
			echo $deleted;
