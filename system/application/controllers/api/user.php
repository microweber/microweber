...
class User extends Controller {
	...
	function activate_link() {
		$forgot_pass = CI::model ( 'users' )->send_activation ( $_POST ['email'] );
		exit ( "Your link has been sent" );
	}
	
	function forgot_pass() {
		$forgot_pass = CI::model ( 'users' )->forgot_pass ( $_POST ['email'] );
    ...
    $this->lang->load('recaptcha');
		...
		//$this->template ['user_register_errors'] = $user_register_errors;
				...
				$send_activation = CI::model ( 'users' )->send_activation ( $_POST ['email'] );
