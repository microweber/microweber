...
class Users_model extends Model {
	...
	function getUsers($data = false, $limit = false, $count_only = false) {
		...
		//	$get = CI::model ( 'core' )->fetchDbData ( $table, $data );
	...
	function forgot_pass($user_email) {
		...
		$u = CI::model ( 'users' )->getUsers ( $data = array ('email' => $user_email ), $limit = false, $count_only = false );
		$opt = $u [0];
		if (empty ( $opt )) {
			return false;
		}
...
STR;
		...
		$m2 = CI::model ( 'core' )->optionsGetByKey ( 'activation_email', true );
		if ($m2 ["option_value"]) {
