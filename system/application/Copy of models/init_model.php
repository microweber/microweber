...
class Init_model extends Model {
	...
	//$result = $db->describeTable(TABLE_PREFIX."options");
	...
	/*	$q = "select option_value from ".TABLE_PREFIX."options where option_key='firecms_db_version' ";
		...
		$firecms_db_version = FIRECMS_DB_VERSION;
		...
		if($firecms_db_version != $curent_db_version){
			...
			$query = (array_values($query));
			$curent_db_version = $query[0];
			//print $curent_db_version;
			if(intval($curent_db_version) == 0){
				$data = array(
				'updated_on'      => date("Y-m-d h:i:s"),
				'option_key'      => 'firecms_db_version',
				'option_value'      => $firecms_db_version
				);
				$this->db->insert(TABLE_PREFIX.'options', $data);
			} else {
				$data = array(
				'updated_on'      => date("Y-m-d h:i:s"),
				'option_value'      => $firecms_db_version
				);
				$this->db->where('option_key', 'firecms_db_version');
				$this->db->update(TABLE_PREFIX.'options', $data);
