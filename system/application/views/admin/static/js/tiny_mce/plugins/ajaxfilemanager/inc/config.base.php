...
if (strval ( $user ) != "y") {
	...
	$the_user = $CI->session->userdata ( 'user_session' );
	//p($the_user);
	$user = ($the_user ["user_id"]);
	if ((intval ( $user ) != 0) and ($the_user ["is_logged"]) == 'yes') {
		$the_user = $CI->session->userdata ( 'user' );
		if (! empty ( $the_user )) {
		
		} else {
			exit ( 'Sorry no access to the file manager :(  Wrong user credentials ' );
		}
		//p ( $the_user );
	} else {
		
		exit ( 'Sorry no access to the file manager :(  Login to the system to get access ' );
	}
} else {
	//var_dump($user);
//
...
if ($the_user ['is_admin'] == 'y') {
	...
	$filespath = $fckEditor_filespath.'users/'.$the_user ['id'].'/';
}

if(is_dir($filespath) == false){
	mkdir_recursive($filespath);
}
...
define ( 'CONFIG_SYS_DEFAULT_PATH', $filespath ); //accept relative path only
define ( 'CONFIG_SYS_ROOT_PATH', $filespath ); //accept relative path only
...
define ( 'CONFIG_UPLOAD_MAXSIZE', 5000 * 1024 * 50 ); //by bytes
...
define ( 'CONFIG_UPLOAD_INVALID_EXTS', 'php,htaccess,asp,cgi,exe,sh,back,cgi,java' );
...
define ( 'CONFIG_DEFAULT_PAGINATION_LIMIT', 100 );
