...
url=t.encode(s.url_converter.call(s.url_converter_scope||t,t.decode(url),'src','script'));return'mce_src="'+url+'"';});if(tinymce.trim(text))
text='<!--\n'+trim(text)+'\n// -->';return'<mce:script'+attribs+'>'+text+'</mce:script>';});h=h.replace(/<style([^>]+|)>([\s\S]*?)<\/style>/gi,function(v,attribs,text){if(text)
...
return-1;nodeVal=container.nodeValue;rng=body.createTextRange();container.nodeValue=nodeVal.substring(0,offset)+invisibleChar+nodeVal.substring(offset);rng.moveToElementText(container.parentNode);rng.findText(invisibleChar);pos=Math.abs(rng.moveStart('character',-0xFFFFF));container.nodeValue=nodeVal;return pos;};if(rng.collapsed){pos=getCharPos(sc,so);ieRng=body.createTextRange();ieRng.move('character',pos);ieRng.select();return;}else{if(sc==ec&&sc.nodeType==3){startPos=getCharPos(sc,so);ieRng.move('character',startPos);ieRng.moveEnd('character',eo-so);ieRng.select();return;}
...
np.appendChild(n);}}}});}
if(s.fix_table_elements){t.onPreProcess.add(function(se,o){each(t.dom.select('p table',o.node).reverse(),function(n){var parent=t.dom.getParent(n.parentNode,'table,p');if(parent.nodeName!='TABLE'){if(isIE)
t.dom.setOuterHTML(n,n.outerHTML);t.dom.split(parent,n);}});});}},setEntities:function(s){var t=this,a,i,l={},re='',v;if(t.entityLookup)
...
return wa[i];}
return null;},serialize:function(n,o){var h,t=this,doc;t._setup();o=o||{};o.format=o.format||'html';n=n.cloneNode(true);t.processObj=o;if(tinymce.isWebKit){doc=n.ownerDocument.implementation.createHTMLDocument("");doc.body.appendChild(doc.importNode(n));}
