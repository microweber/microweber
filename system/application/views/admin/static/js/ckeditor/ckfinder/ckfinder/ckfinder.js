...
var CKFinder = function( basePath, width, height, selectFunction )
{
	...
	this.BasePath = filebrowserBrowserPath ;
...
CKFinder.DEFAULT_BASEPATH = filebrowserBrowserPathAndFilename ;
...
CKFinder.prototype = {
	...
	CreateHtml : function()
	{
		...
		return '<iframe src="' + filebrowserBrowserPathAndFilename + '" width="' + this.Width + '" ' +
...
CKFinder.SetupFCKeditor = function( editorObj, basePath, imageType, flashType )
{
	...
	if ( basePath != null && typeof( basePath ) == 'object' )
	{
		...
		ckfinder = new CKFinder( filebrowserBrowserPath ) ;

	var url = filebrowserBrowserPath ;
	...
	url = filebrowserBrowserPathAndFilename ;
	var qs = ( url.indexOf( "?" ) !== -1 ) ? "&amp;" : "?" ;

	editorObj.Config['LinkBrowserURL'] = filebrowserBrowserPathAndFilename ;
	editorObj.Config['ImageBrowserURL'] = filebrowserBrowserPathAndFilename + qs + 'type=' + ( imageType || 'Images' ) ;
	editorObj.Config['FlashBrowserURL'] = filebrowserBrowserPathAndFilename + qs + 'type=' + ( flashType || 'Flash' ) ;

	var dir = url.substring(0, 1 + url.lastIndexOf("/"));
	editorObj.Config['LinkUploadURL'] = filebrowserBrowserPath + "core/connector/" + ckfinder.ConnectorLanguage + "/connector." 
		+ ckfinder.ConnectorLanguage + "?command=QuickUpload&type=Files" ;
	editorObj.Config['ImageUploadURL'] = filebrowserBrowserPath + "core/connector/" + ckfinder.ConnectorLanguage + "/connector." 
		+ ckfinder.ConnectorLanguage + "?command=QuickUpload&type=" + ( imageType || 'Images' ) ;
	editorObj.Config['FlashUploadURL'] = filebrowserBrowserPath + "core/connector/" + ckfinder.ConnectorLanguage + "/connector." 
