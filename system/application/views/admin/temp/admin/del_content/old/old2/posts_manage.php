...
*/
		...
		d.add(0,-1,'Categories', '<?php print site_url('admin/content/posts_manage/'); ?>'); 
	  ...
	  <?php endif; ?>
		...
		d.add(<?php print $item['id'] ?>,<?php print $item['parent_id'] ?>,'<?php print $is_open ?><?php print addslashes(character_limiter($item['taxonomy_value'], 35, ' ') )?>','<?php print $link ; ?>',false,false,<?php print $thumb ?>,<?php print $thumb ?>);
...
<br />
    ...
    <td><h2>Search</h2>
      <table border="0" cellspacing="0" cellpadding="0" width="100%">
        <tr>
          <td><form action="<?php print site_url('admin/content/posts_manage_do_search_by_keyword/'); ?>" method="post">
                ...
                <tr>
                  <td colspan="6"><table width="100%" border="0" cellspacing="0">
                      <tr>
                        ...
                        <td><input name="keyword" type="text" style="width:80%;" value="<?php print $search_by_keyword ?>" /></td>
                      </tr>
                    </table></td>
                </tr>
                <tr> 
                  <td colspan="6"><table width="100%" border="0" cellspacing="0">
                      <tr>
                        ...
                        <td><input name="tags" id="search_by_tags" type="text" style="width:80%;" value="<?php print $content_selected_tags ?>" /></td>
                      </tr>
                    </table></td>
                </tr>
                <tr>
                  ...
                  <td><?php if($this->core_model->plugins_isPluginLoaded('rss_grabber') == true) : ?>
                    <?php $grabber = new rss_grabber_model(); 
   ...
   ?>
                    ...
                    <label><img src="<?php print_the_static_files_url() ; ?>/icons/feed__arrow.png" alt=" " border="0">From RSS?
                      <select name="is_from_rss">
                        ...
                        <option value=""  <?php if($search_by_is_from_rss == '') : ?> selected="selected" <?php endif; ?> >Any</option>
                        <option value="y" <?php if($search_by_is_from_rss == 'y') : ?> selected="selected" <?php endif; ?> >Yes</option>
                        <option value="n" <?php if($search_by_is_from_rss == 'n') : ?> selected="selected" <?php endif; ?>>No</option>
                        </optgroup>
                        <optgroup label="From Specific RSS feed">
                        <?php foreach($feeds as $item):  ?>
                        <option value="<?php print $item['id'] ?>"   <?php if($search_by_is_from_rss == $item['id']) : ?> selected="selected" <?php endif; ?>  ><?php print addslashes(character_limiter($item['feed_name'],20, '...') )?></option>
                        <?php endforeach; ?>
                        </optgroup>
                        <?php endif; ?>
                        <?php endif; ?>
                    ...
                    <label><img src="<?php print_the_static_files_url() ; ?>/icons/thumb_up.png" alt=" " border="0">Only voted
                      <select name="voted">
                       ...
                       <?php $number = 365*2;
for ($x = 1; $x <= $number; $x++) : ?>
<option value="<?php print $x ?>"  <?php if($selected_voted ==  $x) : ?> selected="selected" <?php endif; ?> ><?php print $x ?> days</option>
<?php endfor; ?>
                      </select>
                    </label></td>
...
function batch_delete_all_items(){
   ...
   <div id="tabs-3">
	...
	?>
        ...
        <label><input name="content_parent" type="radio" value="<?php print $section['id'] ?>"      />
        <input type="hidden" name="content_subtype_value_<?php print $section['id'] ?>"  id="content_subtype_value_<?php print $section['id'] ?>"  value="<?php print $section['content_subtype_value'] ?>" /><span id="content_url_<?php print $section['id'] ?>"><?php print site_url($section['content_url']); ?></span>
        </label>
      ...
      <table border="0" class="tablesorter tables" id="sortedtable" cellspacing="0" cellpadding="0">
        ...
        <tbody id="the_content_items_container">
          ...
          <tr id="content_row_id_<?php print $item['id']; ?>">
            ...
            <td><div style="display:block; overflow:hidden; height:64px; width:64px; vertical-align:middle; text-align:center"> <img src="<?php print   $thumb = $this->content_model->contentGetThumbnailForContentId($item['id'], 64); ?>" height="64" /></div></td>
            <td><?php //print $the_url.'/'.$item['content_url'] ?>
            ...
            <td>
            <?php if($selected_voted == false) : ?><?php $temp = 7 ?><?php else: ?><?php $temp = $selected_voted ?><?php endif; ?>
			<?php print $this->content_model->contentGetVotesCountForContentId($item['id'], $temp); ?>
            </td>
