...
function enc($var) {
	...
	$var = CI::model ( 'core' )->securityEncryptString ( $var );
	return $var;
}
function dec($var) {
	$var = CI::model ( 'core' )->securityDecryptString ( $var );
	return $var;
...
function get_posts($params = false) {
	...
	if ($params ['page_id']) {
		...
		$content1 = CI::model ( 'content' )->contentGetById ( $params ['page_id'] );
...
function get_category_id($category_name) {
	...
	$category_id = CI::model ( 'taxonomy' )->getIdByName ( $category_name );
...
function get_pictures($content_id, $for = 'post') {
	...
	if ($for != 'post') {
		$for = CI::model ( 'core' )->guessDbTable ( $for );
