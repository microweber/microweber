...
class upsRate {
	...
	function getRate($PostalCode, $dest_zip, $service, $length, $width, $height, $weight) {
		...
		// var_dump( $result);
		$pos1 = strpos($result, '<TotalCharges>');
		$pos2 = strpos ($result, '</TotalCharges>' );
		
		
		//var_dump( $pos1,$pos2  );
		$total = substr($result,$pos1, $pos2- $pos1 );
		 //var_dump( $total);
		 
		$pos1 = strpos($total, '<MonetaryValue>');
		$pos2 = strpos ($total, '</MonetaryValue>' );
		
		
		//var_dump( $pos1,$pos2  );
		$total = substr($total,$pos1, $pos2- $pos1 );
		$total  = str_ireplace('<MonetaryValue>','',$total );
		$total  = str_ireplace('</MonetaryValue>','',$total );
