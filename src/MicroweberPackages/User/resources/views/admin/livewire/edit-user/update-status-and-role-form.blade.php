    ...
    <x-microweber-ui::form-section submit="updateProfileInformation">
        ...
        <x-slot name="form">
            ...
            <div class="col-span-6 sm:col-span-4 mt-2">
                ...
                <div class="text-muted"><?php _e('Choose the current role of the user');?>. </div>
            ...
            <div class="col-span-6 sm:col-span-4 mt-2">
                ...
                <div class="text-muted"><?php _e('Choose the current status of this user');?></div>
            ...
            <br />
            <a href="{{api_url('users/export_my_data?user_id=' . $state['id'])}}" class="btn btn-outline-primary"><?php _e('Export user data');?></a>
            <button type="button" class="btn btn-outline-primary" wire:click="$emit('openModal', 'admin::user-tos-log', {{ json_encode(['userId' => $state['id']]) }})"><?php _e('Terms agreement log');?></button>
            <button type="button" class="btn btn-outline-primary" wire:click="$emit('openModal', 'admin::user-login-attempts', {{ json_encode(['userId' => $state['id']]) }})"><?php _e('Login attempts');?></button>
