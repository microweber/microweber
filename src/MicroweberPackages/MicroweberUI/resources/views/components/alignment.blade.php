<div>
    <div class="btn-group d-flex" role="group" aria-label="Alignment">
        <button type="button" class="btn btn-white p-2 btn-sm active">
            <img src="data:image/svg+xml;base64," style="width: 22px; height: 22px;">
        </button>
        <button type="button" class="btn btn-white p-2 btn-sm">
            <img src="data:image/svg+xml;base64," style="width: 22px; height: 22px;">
        </button>
        <button type="button" class="btn btn-white p-2 btn-sm">
            <img src="data:image/svg+xml;base64," style="width: 22px; height: 22px;">
        </button>
    </div>
</div>
