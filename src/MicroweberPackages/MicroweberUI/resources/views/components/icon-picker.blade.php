...
<div class="microweber-ui-icon-picker">
    <div class="btn-group d-flex">
        ...
        @if(!empty($value))
            <button class="btn btn-white" style="font-size:24px">
                {!! $value !!}
            </button>
        @endif

        <button class="btn btn-white" onclick="mw.app.iconPicker.selectIcon('#btn-icon-pick-{{$randId}}')" type="button">
        ...
        @if(!empty($value))
            <button class="btn btn-white" onclick="mw.app.iconPicker.removeIcon('#btn-icon-pick-{{$randId}}')" type="button">
