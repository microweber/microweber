...
<div class="input-group mb-2">
    <input type="text"
           class="form-control js-btn-link-value{{$rand}}"
           placeholder="https://example.com"
           wire:model.debounce.100ms="state.settings.{{ $this->optionName }}">
    <button class="btn js-open-link-editor{{$rand}}" type="button">
        ...
        $('.js-open-link-editor{{$rand}}').click(function () {
            new mw.LinkEditor({
                ...
                .then(function (result){
                    $('.js-btn-link-value{{$rand}}').val(result.url);
                    $('.js-btn-link-value{{$rand}}').trigger('change');
                });
        });
    });
</script>
