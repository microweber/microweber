    ...
    public static function autoloadClass($class): bool
{
    ...
    public static function autoloadClassdddd($class): bool
    {
        $segments = explode('\\', $class);
        if (empty(self::$namespaces)) {
            return false;
        }
        if (isset(self::$notFoundClasses[$class])) {
            return false;
        }
        if (isset(self::$foundClasses[$class])) {
        ...
        while (!empty($segments)) {
            ...
            if (isset(self::$namespaces[$namespace])) {
                foreach (self::$namespaces[$namespace] as $path) {
                    ...
                    $classFile = str_replace('\\', DIRECTORY_SEPARATOR, $relativeClass);
                    $path = rtrim($path, '\\/');
                    $classFile = rtrim($classFile, '\\/');
                    $file = $path . DIRECTORY_SEPARATOR . $classFile . '.php';
                    if (is_file($file)) {
                        self::$foundClasses[$class] = $file;
                        require_once($file);
