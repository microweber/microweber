<div class="mt-3">
    ...
    <div id="js-sortable-items-holder-{{$this->getId()}}">
        @foreach($customField->fieldValue as $fieldValue)
            <div class="d-flex gap-3 mt-3 js-sortable-item" sort-key="{{ $fieldValue->id }}" wire:key="custom-field-vals-{{ $this->id }}-{{ $fieldValue->id }}">
                ...
                <div class="w-full">
                    <x-microweber-ui::input class="mt-1 block w-full" wire:model.live.debounce="inputs.{{ $fieldValue->id }}" />
                ...
                @if (isset($customField->options['as_price_modifier']) && $customField->options['as_price_modifier'])
                    <div class="w-full" style="max-width: 120px;">
                        <x-microweber-ui::input type="number" class="mt-1 block w-full" wire:model.live.debounce="priceModifiers.{{ $fieldValue->id }}" placeholder="0" />
    ...
    <div class="mt-3">
        ...
        <x-microweber-ui::toggle id="as_price_modifier" class="mt-1 block w-full" wire:model.live="state.options.as_price_modifier" />
    </div>
