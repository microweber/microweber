    ...
    @if($moduleInfo && isset($moduleInfo['name']))
        <div class="mw-admin-action-links" id="sidebar-admin-modules-list-tree-item-{{ $item['id'] }}">
            {{-- AI-113 / TICKET-CP (cycle-103 2026-05-09): inline
                 onclick/ondblclick lifted to data-mw-* attributes +
                 delegated listener (mw-modules-tree-handlers.js).
                 Pre-fix the inline shape collided with strict CSP
                 `script-src 'self'` AND apostrophes in `$item['id']`
                 would have terminated the onclick string. --}}
            <span class="cursor-pointer mw-modules-tree-link" data-mw-module-id="{{ $item['id'] }}" data-mw-action="select-and-scroll">
            ...
            <button class="mw-modules-tree-settings" data-mw-module-id="{{ $item['id'] }}" data-mw-action="select" type="button">
