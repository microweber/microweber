...
mw.wysiwyg = {
    ...
    setActiveButtons:function(node){
        ...
        if(typeof css.get !== 'undefined'){
              ...
              var ddval = mw.$(".mw-dropdown_action_font_family");
              if(ddval.length!=0 && ddval.setDropdownValue != undefined){
                    mw.$(".mw-dropdown_action_font_family").setDropdownValue(fam);
              }
         }
    },
    setActiveFontSize:function(node){
        var size = Math.round(parseFloat(mw.CSSParser(node).get.font().size));
		var ddval = mw.$(".mw-dropdown_action_font_size");
		if(ddval.length !=0 && ddval.setDropdownValue != undefined){
            mw.$(".mw-dropdown_action_font_size .mw-dropdown_val").html(size+'px')
		}
    },
         ...
         if(!mw.wysiwyg.started_checking){
             ...
             if(selection.rangeCount>0){
                 ...
                 if(mw.wysiwyg.isFormatElement(common)){
				  ...
				  var ddval = mw.$(".mw-dropdown_action_format");
				  if(ddval.length!=0 && ddval.setDropdownValue != undefined){
                    mw.$(".mw-dropdown_action_format").setDropdownValue(format);
				  }
                 }
                 else{
...
$(mwd).ready(function(){
  mw.$(".mw-dropdown_action_font_family").change(function(){
      var val = $(this).getDropdownValue();
      mw.wysiwyg.fontFamily(val);
  });
  mw.$(".mw-dropdown_action_font_size").change(function(){
      var val = $(this).getDropdownValue();
      mw.wysiwyg.fontSize(val);
      mw.$('.mw-dropdown_val', this).append('px');
  });
  mw.$(".mw-dropdown_action_format").change(function(){
      var val = $(this).getDropdownValue();
