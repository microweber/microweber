...
function load_all_functions_files_for_modules($options = false)
{
    ...
    $modules = mw()->modules_manager->get('ui=any&installed=1&limit=99999');
    $files = array();
    if (!empty($modules)) {
...
{
    return mw()->modules_manager->info($module_name);
}
...
function module($params)
{
    ...
    $res = mw()->modules_manager->load($module_name, $params);
...
{
    return mw()->modules_manager->exists($module_name);
}

function module_url($module_name = false)
{
    return mw()->modules_manager->url($module_name);
}

function module_dir($module_name)
{
    return mw()->modules_manager->dir($module_name);
}

function locate_module($module_name, $custom_view = false, $no_fallback_to_view = false)
{
    return mw()->modules_manager->locate($module_name, $custom_view, $no_fallback_to_view);
}
...
{
    return mw()->modules_manager->uninstall($params);
}
...
function re_init_modules_db()
{
    ...
    //return mw()->modules_manager->update_db();
}
...
{
    return mw()->modules_manager->set_installed($params);
}

function save_module_to_db($data_to_save)
{
    return mw()->modules_manager->save($data_to_save);
}

function get_saved_modules_as_template($params)
{
    return mw()->modules_manager->get_saved_modules_as_template($params);
}

api_expose_admin('delete_module_as_template');
function delete_module_as_template($data)
{
    return mw()->modules_manager->delete_module_as_template($data);
}

api_bind_admin('module/reorder_modules', function ($data) {
    return mw()->modules_manager->reorder_modules($data);
});
