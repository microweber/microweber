...
{
    return mw()->modules_manager->templates($module_name, $template_name, $is_settings);
}
...
{
    return mw()->modules_manager->get($options);
}
...
{
    return mw()->modules_manager->save_module_as_template($data_to_save);
}
...
{
    return mw()->modules_manager->scan_for_modules($options);
}

function scan_for_elements($options = array())
{
    return mw()->modules_manager->scan_for_elements($options);
}

function have_license($module_name = false)
{
    return mw()->modules_manager->license($module_name);
}

function load_module($module_name, $attrs = array())
{
    return mw()->modules_manager->load($module_name, $attrs);
}

function module_css_class($module_name)
{
    mw()->modules_manager->css_class($module_name);
}
...
{
    if (mw()->modules_manager->is_installed('admin/backup')) {
    ...
    if (mw()->modules_manager->is_installed('admin/import')) {
        $active = mw()->url_manager->param('view');
        $cls = '';
...
function get_all_functions_files_for_modules($options = false)
{
    ...
    $installed = mw()->modules_manager->get('ui=any&installed=1');
    $configs = false;
