...
<html>
<head>
    <title><?php _e("Weber installation"); ?></title>
    <meta charset="utf-8">
...
<div class="installholder">
 ...
 <small class="version">v. <?php print WB_VERSION ?></small>
<div class="mw-ui-box">
<div class="mw-ui-box-header">
<a href="http://WeberCMS.com" target="_blank" id="logo">
...
<?php if ($done == false): ?>
  ...
  if ($is_pdo_loaded == false) {
    ...
    if (defined('WB_USERFILES') and is_dir(WB_USERFILES) and !is_writable(WB_USERFILES)) {
        $check_pass = false;
        $must_be = WB_USERFILES;
        $server_check_errors['WB_USERFILES'] = _e("The directory " . WB_USERFILES . " must be writable", true);
    }

    if (defined('WB_CACHE_ROOT_DIR') and is_dir(WB_CACHE_ROOT_DIR) and !is_writable(WB_CACHE_ROOT_DIR)) {
        $check_pass = false;
        $must_be = WB_CACHE_ROOT_DIR;
        $server_check_errors['WB_CACHE_ROOT_DIR'] = _e("The directory " . WB_CACHE_ROOT_DIR . " must be writable", true);
    }
    ...
    if (defined('WB_MEDIA_DIR') and is_dir(WB_MEDIA_DIR) and !is_writable(WB_MEDIA_DIR)) {
        $check_pass = false;
        $must_be = WB_MEDIA_DIR;
        $server_check_errors['WB_MEDIA_DIR'] = _e("The directory " . WB_MEDIA_DIR . " must be writable", true);
    }
    if (defined('WB_APP_PATH') and is_dir(WB_APP_PATH) and !is_writable(WB_APP_PATH)) {
        $check_pass = false;
        $must_be = WB_APP_PATH;
        $server_check_errors['WB_APP_PATH'] = _e("The directory " . WB_APP_PATH . " must be writable", true);
    }
    ...
    <?php if ($check_pass == false): ?>
        <?php if (!empty($server_check_errors)): ?>
            ...
            <h4>
                <?php _e("There are some errors on your server that will prevent Weber from working properly"); ?>
            </h4>
            <ol class="error">
        ...
        <form method="post" id="form_<?php print $rand; ?>" autocomplete="true">
            ...
            <div class="hr"></div>
                ...
                <div class="mw-ui-field-holder">
                    <label class="mw-ui-label">
                        <?php _e("Table prefix"); ?>
                        <span data-help="<?php _e("Change this If you want to install multiple instances of Weber to this database. Only latin letters and numbers are allowed."); ?>"><span class="mw-icon-help-outline mwahi tip"></span></span></label>
