...
- [x] 2026-05-06  [TICKET-D2] feat(a11y): replicate TICKET-D skip-link in remaining public themes (default, Big2) — *DEFERRED to focused per-theme follow-up. Same one-blade-edit pattern as TICKET-D — `<a visually-hidden-focusable>` as first body element + `<main id="main-content" tabindex="-1">` wrapper around `@yield('content')`. Each theme has its own master.blade.php. Tracked here so it cannot rot.*
