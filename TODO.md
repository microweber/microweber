...
- [~] [task-2026-04-29-1a3e1f] You didn't populate the Todo and skipped tests please recheck
  - [ ] (subtask) Revisit LiveEditAddContentBig2Test — re-add product flow with the right required fields (price, content_type=product)
  - [ ] (subtask) Verify existing tests still pass (LiveEditAddContentBig2 + PostModuleAdmin + ProductModuleAdmin + LiveEditPostsListAddPost + LiveEditPostModuleSettingsAddEdit) so adding product back doesn't regress them
