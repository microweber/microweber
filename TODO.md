...
- [x] 2026-05-07  [TICKET-Y] feat(a11y,seo): semantic-HTML sweep across title/text/categories/logo/breadcrumb modules — *DEFERRED to focused semantic-template sprint. Findings: title module has no <h1>/<h2>; text module has no <p>; categories has no <nav aria-label>; logo has alt='' (should read site_name); breadcrumb separator needs aria-hidden. SEO + a11y win, but template-side decision per module (where in the page hierarchy each one sits). Source: agent-test Module Live-Test 2026-05-07 title/text/categories/logo/breadcrumb reports. **CARRY-OVER (per agent-test cycle-14 ack 2026-05-07T09:41): SocialLinks rel=noopener BLOCKER from 2026-05-06 detailed reply still open — every external SocialLinks anchor uses `target="_blank"` without `rel="noopener noreferrer"`. 13-platform tabnabbing surface. Bundle into TICKET-Y as a sub-task so it doesn't slide.**
- [x] 2026-05-07  [TICKET-Z] feat(a11y,forms): form-field a11y bundle (autocomplete + inputmode + type + aria-required) across contact_form/custom_fields/search/newsletter — *DEFERRED to focused form-templates sprint. Findings: missing autocomplete, missing aria-required + native required, missing semantic field types (type='email'/'tel'), missing inputmode='email'/'tel', missing role='search' on search forms, missing enterkeyhint='search', missing aria-live for newsletter feedback, missing aria-label on submit buttons. Note: custom_fields IS the underlying renderer that contact_form composes — fix once, ship twice. Source: agent-test Module Live-Test 2026-05-07 contact_form/custom_fields/search/newsletter reports. **CARRY-OVER (per agent-test cycle-14 ack 2026-05-07T09:41): Tag URL-encoding BLOCKER from 2026-05-06 detailed reply still open — `<a href="{{ $tags_url_base }}?tags={{ $tag }}">` does NOT URL-encode the tag value, so a tag containing `&` or space corrupts the query string. Use `urlencode($tag)`. Bundle into TICKET-Z as a sub-task so it doesn't slide.**
