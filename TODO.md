...
- [x] 2026-04-16  fix: Profile module — FilamentProfilePanelProvider.php deprecated colors()
- [x] 2026-04-16  fix: Checkout module — FilamentCheckoutPanelProvider.php deprecated colors()
- [x] 2026-04-16  fix: Ai module — AgentChatResource.php deprecated colors()
- [x] 2026-04-16  fix: Media module — MediaResource.php deprecated colors()
- [x] 2026-04-16  fix: Invoice module — InvoiceResource.php deprecated colors()
- [x] 2026-04-16  fix: Currency module — ExchangeRateResource.php deprecated colors()
- [x] 2026-04-16  fix: Billing frontend — BillingFilamentFrontendPanelProvider.php deprecated colors()
...
- [x] 2026-04-16  fix the lgoo spacing on the login sccreen [attachment: .autodev/messages/attachments/20260416_102336_2488ebc7/paste-1776324201115.png] [attachment: .autodev/messages/attachments/20260416_102336_bb9d9e5f/paste-1776324213675.png]
...
- [x] 2026-04-22  make the pages smoke tests function on trait and use in on theo ther dusk tests
...
- [x] 2026-04-16  Remove `PostApiRepository` (Modules/Post/Repositories/) — pure BaseRepository CRUD wrapper
- [x] 2026-04-16  Remove `OrderApiRepository` (Modules/Order/Repositories/) — pure BaseRepository CRUD wrapper with events (move events to model observers)
- [x] 2026-04-16  Remove `UserRepository` (src/MicroweberPackages/User/Repositories/) — pure CRUD with events (move events to model observers)
- [x] 2026-04-16  Update all references to use models directly; move event dispatching to Eloquent observers
...
- [x] 2026-04-16  Keep `OptionRepository` as thin cache wrapper
...
- [x] 2026-04-16  Eliminate `CartManager` — merge into model (KEPT: CartManager is a service/facade layer already delegating to CartService, CartTotalsService, CartCouponService — 92 references across 34 files including Checkout, Shipping, Coupons, Shop modules — not a repository duplicate)
...
- [x] 2026-04-19  therei s some border onthe module seddings cards, pls remove the borders and unify the bg color [attachment: .autodev/messages/attachments/20260419_162544_04d55e77/paste-1776605136446.png]
...
- [x] 2026-04-19  the tabs are too small  make tme mre spacous acocotrin to the golden ui [attachment: .autodev/messages/attachments/20260419_192536_08f64f51/paste-1776615918913.png]
...
- [x] 2026-04-22  CORS configuration for third-party frontends
...
- [x] 2026-04-22  Wire a tearDown cleanup that deletes any `landing-test-*` pages and their content rows, so reruns don't accumulate
...
- [x] 2026-04-22  Click Save; assert single `POST /live-edit/save` → 200 with non-empty body
- [x] 2026-04-22  Reload live-edit, confirm all 9 sections still present in correct order
...
- [x] 2026-04-22  Assert no iframe renders an empty fallback (`grep` for `field="layout-skin-1-"` in iframe body across a random sample of 5)
- [x] 2026-04-22  Assert category filter narrows the visible cards (pricing category shows exactly 3 cards)
- [x] 2026-04-22  Assert keyword search finds "hosting" → Pricing skin-2 and hosting-compare page layout
...
- [x] 2026-04-22  Switch back to Bootstrap; assert Bootstrap-specific skins render again (regression guard for adapter state leak)
...
- [x] 2026-04-22  Add a `ColorPaletteFactory` under `tests/Browser/Factories/` that seeds an admin user + a pre-populated `color-palette-test-<ts>` page built with a single Jumbotron + Content + Pricing + Footer (covers header, body, primary-button, section-background touchpoints)
- [x] 2026-04-22  Wire a tearDown cleanup that (a) deletes all `color-palette-test-*` pages, (b) clears `options.template` group entries mutated during the run, (c) invalidates `app()->option_repository->clearCache()` and `app()->cache_manager->delete('options')`, and (d) resets `DuskTestCase::$adminLoggedIn = false` after any logout
...
- [x] 2026-04-22  `tests/Browser/LiveEditColorPaletteArcticFrostTest.php` — arctic-frost pack applies every CSS custom property to `:root`
- [x] 2026-04-22  `tests/Browser/LiveEditColorPaletteBlueberryPieTest.php` — blueberry-pie pack applies every CSS custom property to `:root`
- [x] 2026-04-22  `tests/Browser/LiveEditColorPaletteCitrusSplashTest.php` — citrus-splash pack applies every CSS custom property to `:root`
- [x] 2026-04-22  `tests/Browser/LiveEditColorPaletteCoralPopTest.php` — coral-pop pack applies every CSS custom property to `:root`
- [x] 2026-04-22  `tests/Browser/LiveEditColorPaletteCyberMintTest.php` — cyber-mint pack applies every CSS custom property to `:root`
- [x] 2026-04-22  `tests/Browser/LiveEditColorPaletteForestHazeTest.php` — forest-haze pack applies every CSS custom property to `:root`
...
- [x] 2026-04-22  `tests/Browser/LiveEditColorPaletteUrbanConcreteTest.php` — urban-concrete pack applies every CSS custom property to `:root`
...
- [x] 2026-04-23  Add `LiveEditColorPaletteSwitchNoBleedTest` to a bleed-guard testsuite (pattern established by `LiveEditAdminLoginCacheBleedGuard`) that runs it *after* `LiveEditColorPalettePublicRenderTest` to catch static-state bleed across tests
...
- [x] 2026-04-23  Surface the probe as a single Filament form field (URL input + "Check" button) that renders the detected capabilities, estimated item count, and a "Start import" confirm CTA
- [x] 2026-04-23  Dusk: `LiveAdminWordPressMigrationProbeTest` — paste a public WP URL (fixture served via PHP built-in server against `tests/fixtures/wp/`), assert the probe surfaces the right capabilities and count
...
- [x] 2026-04-23  Unit: `RssFeedImporterTest` with recorded fixtures (`tests/fixtures/wp/feed-*.xml`) covering RSS 2.0, RSS w/ `content:encoded`, Atom, and a broken/partial feed
- [x] 2026-04-23  Dusk: `LiveAdminWordPressMigrationRssTest` — feed URL → completes → imported posts render on the public frontend with correct HTML
...
- [x] 2026-04-23  Support optional application-password auth (WP 5.6+) stored per job, plus anon for public-only content
...
- [x] 2026-04-23  Dusk: `LiveAdminWordPressMigrationWxrTest` — upload WXR → import runs → posts visible on frontend
...
- [x] 2026-04-23  HTML rewriter pass (`HtmlMediaRewriter`) walks imported HTML and replaces every `<img src>`, `<a href>` to the old `wp-content/uploads/` origin with the rehosted URL; leaves off-site links alone
- [x] 2026-04-23  Handle redirects and protocol-relative URLs (`//example.com/...`); skip obvious non-asset links (anchors, mailto:, tel:)
- [x] 2026-04-23  Unit: `MediaRehosterTest` covering dedupe, redirect-following, mime-sniffing fallback, and the protocol-relative case
...
- [x] 2026-04-23  Dusk: `LiveAdminWordPressMigrationPreviewCommitTest` — exclude one item in preview, commit, assert excluded item never reaches live `content`
...
- [x] 2026-04-24  CI smoke: a single GitHub Actions step runs `microweber:import:wordpress http://127.0.0.1:9876 --mode=rss --dry-run --yes` against a WordPress fixture served by PHP's built-in server, asserts non-zero items staged
