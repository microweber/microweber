...
/**
 * Phase-3 color-scheme coverage (per-palette): lavender-fields.
 *
 * Applies the lavender-fields style-pack on :root via the same
...
class LiveEditColorPaletteLavenderFieldsTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'lavender-fields';
    ...
    #[Test]
    public function lavender_fields_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('lavender-fields apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'lavender-fields pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "lavender-fields pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after lavender-fields apply"
                );
                $this->assertNotSame(
                    '',
