...
/**
 * Phase-3 color-scheme coverage (per-palette): arctic-frost.
 *
 * Applies the arctic-frost style-pack on :root via the same
 ...
 */
class LiveEditColorPaletteArcticFrostTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'arctic-frost';
    ...
    #[Test]
    public function arctic_frost_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('arctic-frost apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'arctic-frost pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "arctic-frost pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after arctic-frost apply"
                );
                $this->assertNotSame(
                    '',
