...
/**
 * Phase-3 color-scheme coverage (per-palette): neon-night.
 *
 * Applies the neon-night style-pack on :root via the same
...
class LiveEditColorPaletteNeonNightTest extends DuskTestCase
{
    use AdminLoginTrait;
    ...
    #[Test]
    public function neon_night_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('neon-night apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'neon-night pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "neon-night pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after neon-night apply"
                );
                $this->assertNotSame(
                    '',
