...
/**
 * Phase-3 color-scheme coverage (per-palette): sakura-bloom.
 *
 * Applies the sakura-bloom style-pack on :root via the same
...
class LiveEditColorPaletteSakuraBloomTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'sakura-bloom';
    ...
    #[Test]
    public function sakura_bloom_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('sakura-bloom apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'sakura-bloom pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "sakura-bloom pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after sakura-bloom apply"
                );
                $this->assertNotSame(
                    '',
