...
/**
 * Phase-3 color-scheme coverage (per-palette): coral-pop.
 *
 * Applies the coral-pop style-pack on :root via the same
...
class LiveEditColorPaletteCoralPopTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'coral-pop';
    ...
    #[Test]
    public function coral_pop_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('coral-pop apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'coral-pop pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "coral-pop pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after coral-pop apply"
                );
                $this->assertNotSame(
                    '',
