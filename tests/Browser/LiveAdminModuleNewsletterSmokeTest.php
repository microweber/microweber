...
/**
 * MSET.1 batch — Newsletter module smoke (newsletter settings).
 *
 * Same shape as {@see LiveAdminModuleBtnSmokeTest}: the Newsletter
 * module ships a Filament settings page registered via
 * FilamentRegistry::registerPage(NewsletterModuleSettings::class). Filament-
 * default route slug: /admin/newsletter-module-settings.
 *
 * The smoke round-trips the subscriber-list-id option through the same
 * save_module_option() pipeline the page's Livewire updated()
 * hook calls on every reactive field update — that's the option
 * the public-frontend Newsletter module reads on next mount.
 *
 *   1. Signal #1 + #3 (page OK + no console errors): full
 *      assertPageSmokeOk() probe of /admin/newsletter-module-settings.
 *   2. Signal #2 (subscriber-list-id option save round-trip): direct
 ...
 * safe to re-run.
 */
class LiveAdminModuleNewsletterSmokeTest extends DuskTestCase
{
    use AdminLoginTrait;
    use AssertsSkinConsoleClean;

    private const SETTINGS_SLUG = 'newsletter-module-settings';

    private const MODULE_NAME = 'newsletter';

    private const FIXTURE_OPTION_KEY = 'live_admin_module_newsletter_smoke_list_id';

    private const FIXTURE_OPTION_VALUE = 'live-admin-module-newsletter-smoke-list-1';

    private const FIXTURE_OPTION_GROUP = 'live-admin-module-newsletter-smoke';
    ...
    #[Test]
    public function newsletter_settings_page_loads_and_round_trips_an_option(): void
    {
        $this->purgeFixtureOption();
        ...
        $this->browse(function (Browser $browser): void {
            ...
            // newsletter settings admin (HTTP < 500, no Whoops / Internal
            ...
            $browser->pause(1500);
            $this->assertNoConsoleErrors($browser, 'newsletter settings render');

            // Signal #2 — round-trip the subscriber-list-id option through the
    ...
    /**
     * Save the marker-prefixed subscriber-list-id option through the same
     * save_module_option() helper the Newsletter settings page's
    ...
    private function assertOptionRoundTripPersists(): void
    {
        ...
        $this->assertNotNull(
            $row,
            'save_module_option() must persist an options row for the newsletter '
            . 'module — this is the same code path the Newsletter settings page invokes '
        ...
        $this->assertSame(
            ...
            . 'save_module_option(). The public-frontend Newsletter module reads this '
            . 'exact row; a mismatch here would silently break the operator-configured '
            . 'subscriber-list-id option across every page that embeds the module.'
        );
    }
    ...
    private function assertSaveActionWired(Browser $browser): void
    {
        ...
        $this->assertTrue(
            ...
            'newsletter settings page must render at least one Livewire / Filament '
