...
/**
 * MSET.1 batch — Blog module smoke (blog settings).
 *
 * Same shape as {@see LiveAdminModuleBtnSmokeTest}: the Blog
 * module ships a Filament settings page registered via
 * FilamentRegistry::registerPage(BlogSettings::class). Filament-
 * default route slug: /admin/blog-settings.
 *
 * The smoke round-trips the blog layout option through the same
 * save_module_option() pipeline the page's Livewire updated()
 * hook calls on every reactive field update — that's the option
 * the public-frontend Blog module reads on next mount.
 *
 *   1. Signal #1 + #3 (page OK + no console errors): full
 *      assertPageSmokeOk() probe of /admin/blog-settings.
 *   2. Signal #2 (blog layout option save round-trip): direct
 ...
 * safe to re-run.
 */
class LiveAdminModuleBlogSmokeTest extends DuskTestCase
{
    use AdminLoginTrait;
    use AssertsSkinConsoleClean;

    private const SETTINGS_SLUG = 'blog-settings';

    private const MODULE_NAME = 'blog';

    private const FIXTURE_OPTION_KEY = 'live_admin_module_blog_smoke_layout';

    private const FIXTURE_OPTION_VALUE = 'live-admin-module-blog-smoke-default';

    private const FIXTURE_OPTION_GROUP = 'live-admin-module-blog-smoke';
    ...
    #[Test]
    public function blog_settings_page_loads_and_round_trips_an_option(): void
    {
        $this->purgeFixtureOption();
        ...
        $this->browse(function (Browser $browser): void {
            ...
            // blog settings admin (HTTP < 500, no Whoops / Internal
            ...
            $browser->pause(1500);
            $this->assertNoConsoleErrors($browser, 'blog settings render');

            // Signal #2 — round-trip the blog layout option through the
    ...
    /**
     * Save the marker-prefixed blog layout option through the same
     * save_module_option() helper the Blog settings page's
    ...
    private function assertOptionRoundTripPersists(): void
    {
        ...
        $this->assertNotNull(
            $row,
            'save_module_option() must persist an options row for the blog '
            . 'module — this is the same code path the Blog settings page invokes '
        ...
        $this->assertSame(
            ...
            . 'save_module_option(). The public-frontend Blog module reads this '
            . 'exact row; a mismatch here would silently break the operator-configured '
            . 'blog layout option across every page that embeds the module.'
        );
    }
    ...
    private function assertSaveActionWired(Browser $browser): void
    {
        ...
        $this->assertTrue(
            ...
            'blog settings page must render at least one Livewire / Filament '
