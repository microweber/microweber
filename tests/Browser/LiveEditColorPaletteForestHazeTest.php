...
/**
 * Phase-3 color-scheme coverage (per-palette): forest-haze.
 *
 * Applies the forest-haze style-pack on :root via the same
 ...
 */
class LiveEditColorPaletteForestHazeTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'forest-haze';
    ...
    #[Test]
    public function forest_haze_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('forest-haze apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'forest-haze pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "forest-haze pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after forest-haze apply"
                );
                $this->assertNotSame(
                    '',
