...
/**
 * Phase-3 color-scheme coverage (per-palette): cyber-mint.
 *
 * Applies the cyber-mint style-pack on :root via the same
 ...
 */
class LiveEditColorPaletteCyberMintTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'cyber-mint';
    ...
    #[Test]
    public function cyber_mint_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('cyber-mint apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'cyber-mint pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "cyber-mint pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after cyber-mint apply"
                );
                $this->assertNotSame(
                    '',
