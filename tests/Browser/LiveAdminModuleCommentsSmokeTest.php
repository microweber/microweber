...
/**
 * MSET.1 batch — Comments module smoke (comments settings).
 *
 * Same shape as {@see LiveAdminModuleBtnSmokeTest}: the Comments
 * module ships a Filament settings page registered via
 * FilamentRegistry::registerPage(CommentsModuleSettings::class). Filament-
 * default route slug: /admin/comments-module-settings.
 *
 * The smoke round-trips the moderation toggle through the same
 * save_module_option() pipeline the page's Livewire updated()
 * hook calls on every reactive field update — that's the option
 * the public-frontend Comments module reads on next mount.
 *
 *   1. Signal #1 + #3 (page OK + no console errors): full
 *      assertPageSmokeOk() probe of /admin/comments-module-settings.
 *   2. Signal #2 (moderation toggle save round-trip): direct
 ...
 * safe to re-run.
 */
class LiveAdminModuleCommentsSmokeTest extends DuskTestCase
{
    use AdminLoginTrait;
    use AssertsSkinConsoleClean;

    private const SETTINGS_SLUG = 'comments-module-settings';

    private const MODULE_NAME = 'comments';

    private const FIXTURE_OPTION_KEY = 'live_admin_module_comments_smoke_moderate';

    private const FIXTURE_OPTION_VALUE = 'y';

    private const FIXTURE_OPTION_GROUP = 'live-admin-module-comments-smoke';
    ...
    #[Test]
    public function comments_settings_page_loads_and_round_trips_an_option(): void
    {
        $this->purgeFixtureOption();
        ...
        $this->browse(function (Browser $browser): void {
            ...
            // comments settings admin (HTTP < 500, no Whoops / Internal
            ...
            $browser->pause(1500);
            $this->assertNoConsoleErrors($browser, 'comments settings render');

            // Signal #2 — round-trip the moderation toggle through the
    ...
    /**
     * Save the marker-prefixed moderation toggle through the same
     * save_module_option() helper the Comments settings page's
    ...
    private function assertOptionRoundTripPersists(): void
    {
        ...
        $this->assertNotNull(
            $row,
            'save_module_option() must persist an options row for the comments '
            . 'module — this is the same code path the Comments settings page invokes '
        ...
        $this->assertSame(
            ...
            . 'save_module_option(). The public-frontend Comments module reads this '
            . 'exact row; a mismatch here would silently break the operator-configured '
            . 'moderation toggle across every page that embeds the module.'
        );
    }
    ...
    private function assertSaveActionWired(Browser $browser): void
    {
        ...
        $this->assertTrue(
            ...
            'comments settings page must render at least one Livewire / Filament '
