...
/**
 * Phase-3 color-scheme coverage (per-palette): robocop.
 *
 * Applies the robocop style-pack on :root via the same
...
class LiveEditColorPaletteRobocopTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'robocop';
    ...
    #[Test]
    public function robocop_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('robocop apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'robocop pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "robocop pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after robocop apply"
                );
                $this->assertNotSame(
                    '',
