...
/**
 * Phase-3 color-scheme coverage (per-palette): midnight-indigo.
 *
 * Applies the midnight-indigo style-pack on :root via the same
 ...
 */
class LiveEditColorPaletteMidnightIndigoTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'midnight-indigo';
    ...
    #[Test]
    public function midnight_indigo_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('midnight-indigo apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'midnight-indigo pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "midnight-indigo pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after midnight-indigo apply"
                );
                $this->assertNotSame(
                    '',
