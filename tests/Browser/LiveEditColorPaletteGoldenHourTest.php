...
/**
 * Phase-3 color-scheme coverage (per-palette): golden-hour.
 *
 * Applies the golden-hour style-pack on :root via the same
 ...
 */
class LiveEditColorPaletteGoldenHourTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'golden-hour';
    ...
    #[Test]
    public function golden_hour_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('golden-hour apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'golden-hour pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "golden-hour pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after golden-hour apply"
                );
                $this->assertNotSame(
                    '',
