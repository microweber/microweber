...
/**
 * MSET.1 batch — Products module smoke (products settings).
 *
 * Same shape as {@see LiveAdminModuleBtnSmokeTest}: the Products
 * module ships a Filament settings page registered via
 * FilamentRegistry::registerPage(ProductsModuleSettings::class). Filament-
 * default route slug: /admin/products-module-settings.
 *
 * The smoke round-trips the product-list layout option through the same
 * save_module_option() pipeline the page's Livewire updated()
 * hook calls on every reactive field update — that's the option
 * the public-frontend Products module reads on next mount.
 *
 *   1. Signal #1 + #3 (page OK + no console errors): full
 *      assertPageSmokeOk() probe of /admin/products-module-settings.
 *   2. Signal #2 (product-list layout option save round-trip): direct
 ...
 * safe to re-run.
 */
class LiveAdminModuleProductsSmokeTest extends DuskTestCase
{
    use AdminLoginTrait;
    use AssertsSkinConsoleClean;

    private const SETTINGS_SLUG = 'products-module-settings';

    private const MODULE_NAME = 'shop/products';

    private const FIXTURE_OPTION_KEY = 'live_admin_module_products_smoke_layout';

    private const FIXTURE_OPTION_VALUE = 'live-admin-module-products-smoke-grid';

    private const FIXTURE_OPTION_GROUP = 'live-admin-module-products-smoke';
    ...
    #[Test]
    public function products_settings_page_loads_and_round_trips_an_option(): void
    {
        $this->purgeFixtureOption();
        ...
        $this->browse(function (Browser $browser): void {
            ...
            // products settings admin (HTTP < 500, no Whoops / Internal
            ...
            $browser->pause(1500);
            $this->assertNoConsoleErrors($browser, 'products settings render');

            // Signal #2 — round-trip the product-list layout option through the
    ...
    /**
     * Save the marker-prefixed product-list layout option through the same
     * save_module_option() helper the Products settings page's
    ...
    private function assertOptionRoundTripPersists(): void
    {
        ...
        $this->assertNotNull(
            $row,
            'save_module_option() must persist an options row for the shop/products '
            . 'module — this is the same code path the Products settings page invokes '
        ...
        $this->assertSame(
            ...
            . 'save_module_option(). The public-frontend Products module reads this '
            . 'exact row; a mismatch here would silently break the operator-configured '
            . 'product-list layout option across every page that embeds the module.'
        );
    }
    ...
    private function assertSaveActionWired(Browser $browser): void
    {
        ...
        $this->assertTrue(
            ...
            'products settings page must render at least one Livewire / Filament '
