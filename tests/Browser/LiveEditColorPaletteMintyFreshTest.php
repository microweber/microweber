...
/**
 * Phase-3 color-scheme coverage (per-palette): minty-fresh.
 *
 * Applies the minty-fresh style-pack on :root via the same
 ...
 */
class LiveEditColorPaletteMintyFreshTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'minty-fresh';
    ...
    #[Test]
    public function minty_fresh_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('minty-fresh apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'minty-fresh pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "minty-fresh pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after minty-fresh apply"
                );
                $this->assertNotSame(
                    '',
