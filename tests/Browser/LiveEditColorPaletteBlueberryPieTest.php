...
/**
 * Phase-3 color-scheme coverage (per-palette): blueberry-pie.
 *
 * Applies the blueberry-pie style-pack on :root via the same
 ...
 */
class LiveEditColorPaletteBlueberryPieTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'blueberry-pie';
    ...
    #[Test]
    public function blueberry_pie_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('blueberry-pie apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'blueberry-pie pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "blueberry-pie pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after blueberry-pie apply"
                );
                $this->assertNotSame(
                    '',
