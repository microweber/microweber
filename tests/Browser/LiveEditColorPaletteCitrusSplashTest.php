...
/**
 * Phase-3 color-scheme coverage (per-palette): citrus-splash.
 *
 * Applies the citrus-splash style-pack on :root via the same
...
class LiveEditColorPaletteCitrusSplashTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'citrus-splash';
    ...
    #[Test]
    public function citrus_splash_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('citrus-splash apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'citrus-splash pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "citrus-splash pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after citrus-splash apply"
                );
                $this->assertNotSame(
                    '',
