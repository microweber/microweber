...
/**
 * MSET.1 batch — Category module smoke (category settings).
 *
 * Same shape as {@see LiveAdminModuleBtnSmokeTest}: the Category
 * module ships a Filament settings page registered via
 * FilamentRegistry::registerPage(CategoryModuleSettings::class). Filament-
 * default route slug: /admin/category-module-settings.
 *
 * The smoke round-trips the data-source toggle through the same
 * save_module_option() pipeline the page's Livewire updated()
 * hook calls on every reactive field update — that's the option
 * the public-frontend Category module reads on next mount.
 *
 *   1. Signal #1 + #3 (page OK + no console errors): full
 *      assertPageSmokeOk() probe of /admin/category-module-settings.
 *   2. Signal #2 (data-source toggle save round-trip): direct
 ...
 * safe to re-run.
 */
class LiveAdminModuleCategorySmokeTest extends DuskTestCase
{
    use AdminLoginTrait;
    use AssertsSkinConsoleClean;

    private const SETTINGS_SLUG = 'category-module-settings';

    private const MODULE_NAME = 'categories';

    private const FIXTURE_OPTION_KEY = 'live_admin_module_category_smoke_data_use_from_post';

    private const FIXTURE_OPTION_VALUE = 'y';

    private const FIXTURE_OPTION_GROUP = 'live-admin-module-category-smoke';
    ...
    #[Test]
    public function category_settings_page_loads_and_round_trips_an_option(): void
    {
        $this->purgeFixtureOption();
        ...
        $this->browse(function (Browser $browser): void {
            ...
            // category settings admin (HTTP < 500, no Whoops / Internal
            ...
            $browser->pause(1500);
            $this->assertNoConsoleErrors($browser, 'category settings render');

            // Signal #2 — round-trip the data-source toggle through the
    ...
    /**
     * Save the marker-prefixed data-source toggle through the same
     * save_module_option() helper the Category settings page's
    ...
    private function assertOptionRoundTripPersists(): void
    {
        ...
        $this->assertNotNull(
            $row,
            'save_module_option() must persist an options row for the categories '
            . 'module — this is the same code path the Category settings page invokes '
        ...
        $this->assertSame(
            ...
            . 'save_module_option(). The public-frontend Category module reads this '
            . 'exact row; a mismatch here would silently break the operator-configured '
            . 'data-source toggle across every page that embeds the module.'
        );
    }
    ...
    private function assertSaveActionWired(Browser $browser): void
    {
        ...
        $this->assertTrue(
            ...
            'category settings page must render at least one Livewire / Filament '
