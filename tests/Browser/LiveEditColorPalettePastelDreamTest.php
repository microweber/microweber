...
/**
 * Phase-3 color-scheme coverage (per-palette): pastel-dream.
 *
 * Applies the pastel-dream style-pack on :root via the same
 ...
 */
class LiveEditColorPalettePastelDreamTest extends DuskTestCase
{
    use AdminLoginTrait;
    use CleansColorPaletteTestFixtures;
    use LiveEditColorPaletteTrait;

    private const PALETTE_SLUG = 'pastel-dream';
    ...
    #[Test]
    public function pastel_dream_applies_every_css_custom_property_to_root(): void
    {
        $fixture = ColorPaletteFactory::make('pastel-dream apply');

        $packs = array_column($this->listColorPalettes(), null, 'slug');
        $this->assertArrayHasKey(
            self::PALETTE_SLUG,
            $packs,
            'pastel-dream pack must be discoverable on disk'
        );
        $pack = $packs[self::PALETTE_SLUG];
        $this->assertNotEmpty(
            $pack['properties'],
            "pastel-dream pack must declare at least one CSS variable"
        );
        ...
        $this->browse(function (Browser $browser) use ($fixture, $pack) {
            ...
            foreach ([
                ...
                $this->assertArrayHasKey(
                    $coreVar,
                    $after,
                    "Expected core variable {$coreVar} on :root after pastel-dream apply"
                );
                $this->assertNotSame(
                    '',
