...
/**
 * MSET.1 batch — CartAdd module smoke (cart-add settings).
 *
 * Same shape as {@see LiveAdminModuleBtnSmokeTest}: the CartAdd
 * module ships a Filament settings page registered via
 * FilamentRegistry::registerPage(CartAddModuleSettings::class). Filament-
 * default route slug: /admin/cart-add-module-settings.
 *
 * The smoke round-trips the add-to-cart button text through the same
 * save_module_option() pipeline the page's Livewire updated()
 * hook calls on every reactive field update — that's the option
 * the public-frontend CartAdd module reads on next mount.
 *
 *   1. Signal #1 + #3 (page OK + no console errors): full
 *      assertPageSmokeOk() probe of /admin/cart-add-module-settings.
 *   2. Signal #2 (add-to-cart button text save round-trip): direct
 ...
 * safe to re-run.
 */
class LiveAdminModuleCartAddSmokeTest extends DuskTestCase
{
    use AdminLoginTrait;
    ...
    private const MODULE_NAME = 'shop/cart_add';

    private const FIXTURE_OPTION_KEY = 'live_admin_module_cart_add_smoke_button_text';

    private const FIXTURE_OPTION_VALUE = 'Add to bag — cart-add smoke';

    private const FIXTURE_OPTION_GROUP = 'live-admin-module-cart-add-smoke';
    ...
    #[Test]
    public function cart_add_settings_page_loads_and_round_trips_an_option(): void
    {
        $this->purgeFixtureOption();
        ...
        $this->browse(function (Browser $browser): void {
            ...
            // cart-add settings admin (HTTP < 500, no Whoops / Internal
            ...
            $browser->pause(1500);
            $this->assertNoConsoleErrors($browser, 'cart-add settings render');

            // Signal #2 — round-trip the add-to-cart button text through the
    ...
    /**
     * Save the marker-prefixed add-to-cart button text through the same
     * save_module_option() helper the CartAdd settings page's
    ...
    private function assertOptionRoundTripPersists(): void
    {
        ...
        $this->assertNotNull(
            $row,
            'save_module_option() must persist an options row for the shop/cart_add '
            . 'module — this is the same code path the CartAdd settings page invokes '
        ...
        $this->assertSame(
            ...
            . 'save_module_option(). The public-frontend CartAdd module reads this '
            . 'exact row; a mismatch here would silently break the operator-configured '
            . 'add-to-cart button text across every page that embeds the module.'
        );
    }
    ...
    private function assertSaveActionWired(Browser $browser): void
    {
        ...
        $this->assertTrue(
            ...
            'cart-add settings page must render at least one Livewire / Filament '
