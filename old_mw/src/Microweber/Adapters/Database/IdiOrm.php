...
class IdiOrm
{
    ...
    function setup($connection_settings = false)
    {
        ...
        if(isset($db['host']) and isset($db['dbname'])){
            ORM::configure('mysql:host=' . $db['host'] . ';dbname=' . $db['dbname']);
        }
        if(isset($db['user'])){
    ...
    public function query($q, $connection_settings = false)
    {
        ...
        if($connection_settings != false){
            $this->setup($connection_settings);
        }
        $content_table = $this->table_prefix.'content';
        ORM::configure('error_mode', \PDO::ERRMODE_WARNING);

        $return = ORM::for_table($content_table)->raw_query($q, false)->find_array();
        //$return = ORM::raw_execute($q, array())->find_array();
        //$return = ORM::();
        return $return;
