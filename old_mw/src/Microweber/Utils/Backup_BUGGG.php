...
api_expose('Utils\Backup\create_template_export');
...
class Backup
{
    ...
    function __construct($app = null)
    {
        ...
        api_expose('Microweber\Utils\Backup\create_template_export');
    ...
    function cronjob($params = false)
    {
        ...
        if ($cache_content == false or empty($cache_content)) {
            ...
            $export_fn = str_replace('backup_', 'export_', $bak_fn);
            ...
            if (is_array($backup_actions)) {
                ...
                foreach ($backup_actions as $key => $item) {
                    ...
                    if ($i > $limit_per_turn or $cache_lock == $item) {
                        ...
                        if ($item == 'make_db_backup' or $item = 'make_db_template_export') {
                            ...
                            } elseif ($item == 'make_db_template_export') {
                               // $db_file = $this->create($bak_fn . '.sql', 'export_template');
                                $db_file = $this->create($bak_fn . '.sql');
                            }
    ...
    function create($filename = false, $export_type = false)
    {
        ...
        foreach ($tables as $table) {
            ...
            if ($export_type == 'export_template') {
            ...
            if ($table != false and $is_cms_table) {
                ...
                if ($table == MW_TABLE_PREFIX . 'categories_items') {
...
//		fclose($handle);
    ...
    function create_full($backup_type = false)
    {
        ...
        if ($backup_type == 'template_export') {
            $backup_actions[] = 'make_db_template_export';
        ...
        if (!isset($folders) or !is_array($folders)) {
            $folders = array();
        }
...
//
//            $more_folders = MW_TEMPLATES_DIR . $the_active_site_template . DS;
//            if(is_dir($more_folders)){
//            $more_folders = \rglob($more_folders . '*', GLOB_NOSORT);
//            if (!empty($more_folders)) {
//                $folders = array_merge($folders, $more_folders);
//            }
//            }
    ...
    public function create_template_export()
    {
        return $this->create_full('template_export');
    }
    ...
    public function get($keyword = false)
    {
        ...
        $here = $this->get_bakup_location();
        if ($keyword != false and is_string($keyword)) {
            $keyword = trim($keyword);
            $files = glob("$here*$keyword{*.sql,*.zip}", GLOB_BRACE);
