...
{{-- opacity-0 group-hover:opacity-100 --}}
    ...
    <div class="flex flex-row sm:flex-row items-center mt-2 sm:mt-0 w-full sm:w-auto justify-start sm:justify-end gap-3">
        @if($content->content_type === 'product')
            ...
            <span class="inline-flex items-center gap-1 rounded-md px-2.5 py-1 text-sm font-medium {{ $stockColor }}">
                {{ svg($stockIcon, 'w-4 h-4') }}
                {{ $stockLabel }}
            </span>
        @endif
        ...
        <a class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" href="{{$content->link()}}?editmode=y" target="_top" data-bs-toggle="tooltip" aria-label="Live edit" data-bs-original-title="Customize" x-data="{}" x-tooltip="{
            content: 'Live Edit',
            theme: $store.theme,
        }">
        ...
        <a class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" href="{{$content->editLink()}}" target="_top" data-bs-toggle="tooltip" aria-label="Edit" data-bs-original-title="Edit" x-data="{}" x-tooltip="{
            content: 'Settings',
            theme: $store.theme,
        }">
