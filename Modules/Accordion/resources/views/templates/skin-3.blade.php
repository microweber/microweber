...
<div id="mw-accordion-module-{{ $params['id'] }}">
    @foreach ($accordion as $key => $slide)
        ...
        <div class="card">
            <div class="card-header" id="header-item-{{ $params['id'] }}-{{ $edit_field_key }}">
                <h5 class="mb-0">
                    <button class="btn btn-link" data-bs-toggle="collapse" data-bs-target="#collapse-accordion-item-{{ $params['id'] }}-{{ $edit_field_key }}-{{ $key }}" aria-expanded="true" aria-controls="collapse-accordion-item-{{ $params['id'] }}-{{ $edit_field_key }}-{{ $key }}">
