        ...
        @if ($teamcard->count() > 0)
            ...
            <div class="col-xl-3 col-md-6 col-sm-8 col-12 mx-auto d-flex justify-content-center align-items-center py-4">
                <div class="flower-card card w-100 mx-2" style="border-radius: 0 20px 0 20px;">
                    @if ($member['file'])
<img loading="lazy" class="flower-team-card-img card-img-top img-fluid" style="height: 350px;
  object-fit: cover;" src="{{ thumbnail($member['file'], 800) }}" alt="{{ $member['name'] ?? __('Team member') }}"/>
                    @else
                        <img loading="lazy" class="flower-team-card-img card-img-top img-fluid" src="{{ asset('modules/teamcard/default-content/default-image.svg') }}" alt="{{ $member['name'] ?? __('Team member') }}"/>
                    @endif
