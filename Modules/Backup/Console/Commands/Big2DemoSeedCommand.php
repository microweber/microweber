...
class Big2DemoSeedCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        // Cycle-165 / wave3-f (2026-05-10): idempotent upsert. The
        // cycle-N behaviour required `--replace` to overwrite an
        // existing page or warned-and-exited; PM Wave 3 brief is
        // "make idempotent" — re-runs should always produce the same
        // outcome without flags. New default: upsert content body in
        // place if a page with this slug exists. `--replace` keeps
        // the harder-delete-then-recreate semantics for callers that
        // want to bust a stale page id (rare).
        $existing = Content::query()->where('url', $slug)->where('content_type', 'page')->first();
        if ($existing !== null && $replace) {
            $existing->delete();
            $existing = null;
            $this->line("Deleted prior page (--replace).");
        }

        $page = $existing ?? new Content();
        $page->title = $title;
        $page->url = $slug;
        ...
        $page->save();
        if ($existing !== null) {
            $this->line("Updated existing page id={$page->id} in place (idempotent re-run).");
        }
