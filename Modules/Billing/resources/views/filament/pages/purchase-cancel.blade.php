<x-filament-panels::page>
    <div class="w-full min-h-[60vh] flex items-center justify-center">
        <div class="bg-white dark:bg-[#1f2937] shadow-2xl rounded-2xl p-8 max-w-lg w-full">
            ...
            <h1 class="text-2xl font-bold text-red-700 dark:text-white mb-2 text-center">Purchase Cancelled</h1>
            <p class="text-gray-600 dark:text-gray-400 text-center mb-6">Your purchase was cancelled.</p>

            @if($order)
                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 mb-6">
                    <h2 class="text-sm font-semibold text-gray-800 dark:text-white mb-2">Cancelled Order</h2>
                    <p class="text-gray-600 dark:text-gray-400">
                        Order #{{ $order['id'] }} - {{ $order['currency'] }} ${{ number_format($order['amount'], 2) }}
                    </p>
                </div>
            @endif
