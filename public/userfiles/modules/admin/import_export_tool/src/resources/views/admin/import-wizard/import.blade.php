        ...
        <div class="mx-auto col-md-10">
            ...
            <table class="table">
                        ...
                        <select class="form-control" wire:model.live="import_feed.primary_key" id="feed_primary_key">
                            ...
                            @if ($this->import_feed['import_to'] == 'products')
                                ...
                                <optgroup label="Content Data">
                                    ...
                                    <option value="content_data.barcode"
                                            @if($primaryKeyIsMapped) disabled="disabled" @endif>Barcode
                                    </option>
                                    <option value="content_data.mpn"
                                            @if($primaryKeyIsMapped) disabled="disabled" @endif>MPN
                                    </option>
                                </optgroup>
                                ...
                                @php
                                    ...
                                    if (!empty($findContentData)) {
                                        foreach ($findContentData as $field) {
                                            if (array_key_exists($field->field_name, \Modules\Product\Models\Product::$contentDataDefault)) {
                                                continue;
                                            }
                                ...
                                @if(!empty($dbFields))
                                    <optgroup label="Database Fields">
                                        @foreach($dbFields as $dbField)
                                            <option value="content_data.{{$dbField}}"
                                                    @if($primaryKeyIsMapped) disabled="disabled" @endif>{{$dbField}}</option>
                                        @endforeach
                                    </optgroup>
                                @endif
                                ...
                                @if(!empty($customContentDataFields))
                                    <optgroup label="Custom Content Data Fields">
                                        @foreach($customContentDataFields as $mappedTagKey=>$mappedTagName)
                                            <option value="{{$mappedTagName}}"
                                                    @if($primaryKeyIsMapped) disabled="disabled" @endif>{{$mappedTagName}}</option>
                                        @endforeach
                                    </optgroup>
                                @endif
                ...
                <tr>
                    ...
                    <td>
                        <select class="form-control" id="feed_parent_page" wire:model.live="import_feed.parent_page">
                            <option value="0"
                                    <?php if ((0 == $import_feed['parent_page'])): ?> selected="selected" <?php endif; ?>><?php _e("None"); ?></option>
                            <?php
                            $ptOpts = array();
                ...
                <tr>
                    ...
                    <td>
                        <input type="radio" id="feed_download_image_1" value="1"
                               wire:model.live="import_feed.download_images">
                        <label for="feed_download_image_1">Yes</label>

                        <input type="radio" id="feed_download_image_0" value="0"
                               wire:model.live="import_feed.download_images">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <select class="form-control" id="feed_parts" wire:model.live="import_feed.split_to_parts">
                            ...
                            <option value="10">10 part(s) @if($recomendedSipltToParts == 10)
                                    (Recommended)
                                @endif</option>
                            <option value="20">30 part(s) @if($recomendedSipltToParts == 30)
                                    (Recommended)
                                @endif</option>
                            <option value="30">60 part(s) @if($recomendedSipltToParts == 60)
                                    (Recommended)
                                @endif</option>
                            <option value="100">100 part(s) @if($recomendedSipltToParts == 100)
                                    (Recommended)
                                @endif</option>
                            <option value="200">200 part(s) @if($recomendedSipltToParts == 200)
                                    (Recommended)
                                @endif</option>
                            <option value="500">500 part(s) @if($recomendedSipltToParts == 500)
                                    (Recommended)
                                @endif</option>
                        </select>
