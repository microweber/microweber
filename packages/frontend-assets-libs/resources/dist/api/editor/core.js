    ...
    MWEditor.core = {
        capsulatedField: function (options) {
            ...
            if (this.settings.color === "inherit") {
                setTimeout(() => {
                    this.field.style.color = getComputedStyle(
            ...
            scope.frame.addEventListener("load", function () {
                ...
                scope.frame.contentDocument.body.style.background =
                    "transparent";
                scope.frame.contentDocument.documentElement.style.margin = 0;
                scope.frame.contentDocument.documentElement.style.padding = 0;
                scope.frame.contentDocument.documentElement.style.overflow =
                    "hidden";
                scope.frame.contentDocument.documentElement.style.background =
                    "transparent";
            });
        },
        button: function (config) {
            ...
            var defaults = {
                ...
                props: {
                    className:
                        "mdi mw-editor-controller-component mw-editor-controller-button",
                    type: "button",
                },
            };
            ...
            if (node) {
                ...
                if (settings.props && settings.props.tooltip) {
        ...
        colorPicker: function (config) {
            ...
            const escapeHandle = (e) =>
                e.key === "Escape" ? (dlg ? dlg.hide() : "") : undefined;

            el.get(0).ownerDocument.addEventListener("keydown", escapeHandle);
            if (mw.top().app && mw.top().app.canvas) {
                mw.top()
                    .app.canvas.getDocument()
                    .addEventListener("keydown", escapeHandle);
            }

            el.on("click", function (e) {
                var ok = mw.$(
                    `<button class="mw-admin-action-links mw-adm-liveedit-tabs mw-liveedit-button-animation-component">${mw.lang(
                        "OK"
                    )}</button>`
                );
                var cancel = mw.$(
                ...
                dlg = mw.top().dialog({
                    ...
                    beforeRemove: function () {
                        if (config.api) {
                            var fn = config.api.elementNode(
                                config.api.getSelection().focusNode
                            );
                            if (fn) {
                                fn.ownerDocument.body.classList.remove(
                                    "mw-le--hide-selection"
                                );
                            }
                        }
                        if (changeStart) {
                            ...
                            if (
                                typeof mw.top().app !== "undefined" &&
                                typeof mw.top().app.templateSettings !==
                                    "undefined" &&
                                typeof mw.top().app.templateSettings
                                    .colorPaletteManager !== "undefined"
                            ) {
                                mw.top().app.templateSettings.colorPaletteManager.addToPalette(
                                    vall
                                );
                            }
                        }
                ...
                if (config.api) {
                    ...
                    if (fn) {
                        fn.ownerDocument.body.classList.add(
                            "mw-le--hide-selection"
                        );
                    }
                }
                ...
                mw.element(
                    ".a-color-picker-row.a-color-picker-palette .a-color-picker-palette-color:not(.a-color-picker-palette-color-picker)",
                    dlg.container
                ).on("click", function () {
                    if (vall) {
                        el.trigger("change", vall);
        ...
        dropdown: function (options) {
            ...
            this.select = MWEditor.core.element({
                props: {
                    className:
                        "mw-editor-controller-component mw-editor-controller-component-select",
                },
            });
                    ...
                    doc.body.addEventListener("click", function (e) {
                        if (
                            !mw.tools.hasParentsWithClass(
                                e.target,
                                "mw-editor-controller-component-select"
                            )
                        ) {
                            mw.element(
            ...
            if (options.customValue) {
                displayValObj = new MWEditor.core.capsulatedField({
                    ...
                    props: {
                        className:
                            (options.icon ? "mdi-" + options.icon + " " : "") +
                            "mw-editor-select-display-value",
                ...
                displayValNode = MWEditor.core.button({
                    props: {
                        className:
                            ...
                            " mw-editor-select-display-value",
                        innerHTML:
                            '<span class="mw-editor-select-display-value-content">' +
                            (options.placeholder || "") +
                            "</span>",
                        tooltip:
                            options.tooltip ||
            ...
            var valueHolder = MWEditor.core.element({
                props: {
                    className:
                        "mw-editor-controller-component-select-values-holder",
                },
            });
            ...
            this.setData = function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    (function (dt) {
                        var opt = MWEditor.core._dropdownOption(
                            dt,
                            options.eachOption
                        );
                        ...
                        events.forEach((ev) => {
                            opt.get(0).addEventListener(ev, function () {
                                lscope.select.trigger("change", dt);
                                const active = opt.get(0).closest(".active");
                                if (active) {
            ...
            var _handleClick = function (_this) {
                var wrapper = mw.tools.firstParentWithClass(
                    _this,
                    "mw-editor-wrapper"
                );
                if (wrapper) {
                ...
                lscope.select
                    ...
                    .ownerDocument.querySelectorAll(
                        ".mw-bar-control-item.active, .mw-editor-controller-component.active"
                    )
                    .forEach((node) => {
                ...
                mw.element(".mw-editor-controller-component-select").each(
                    function () {
            ...
            lscope.select.on("mousedown", function (e) {
                if (!e.target.closest(".mw-editor-dropdown-option")) {
                    e.stopPropagation();
                    _handleClick(this);
                }
            });
            lscope.select.on("touchstart", function (e) {
                if (!e.target.closest(".mw-editor-dropdown-option")) {
                    e.stopPropagation();
                    _handleClick(this);
                }
        ...
        _preSelect: function (node) {
            var all = document.querySelectorAll(
                ".mw-editor-controller-component-select.active, .mw-bar-control-item-group.active"
            );
            var parent = mw.tools.firstParentOrCurrentWithAnyOfClasses(
                node ? node.parentNode : null,
                [
                    "mw-editor-controller-component-select",
                    "mw-bar-control-item-group",
                ]
            );
            var i = 0,
                l = all.length;
