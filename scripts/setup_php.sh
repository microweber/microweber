...
ensure_xdebug() {
  ...
  if php -m 2>/dev/null | grep -qi '^xdebug$'; then
    ok "Xdebug already loaded: $(php -r 'echo xdebug_info(true) ? "" : "";' 2>/dev/null || php -r 'echo phpversion("xdebug");' 2>/dev/null)"
    return
  fi

  # Already installed but not loaded (INI disabled)?
  local xdebug_so
  xdebug_so="$(php -r 'echo PHP_EXTENSION_DIR;' 2>/dev/null)/xdebug.so"
  if [ -f "$xdebug_so" ]; then
    ok "Xdebug .so present at ${xdebug_so} (not loaded — check your php.ini/conf.d)"
    return
  fi
  ...
  if $SUDO apt-get install -y "php${PHP_VERSION}-xdebug"; then
    ...
    # Write a minimal coverage-mode ini drop-in if one doesn't exist yet.
    local ini_dir
    ini_dir="$(php -r 'echo PHP_SYSCONFDIR;' 2>/dev/null)/conf.d"
    local ini_file="${ini_dir}/99-xdebug-coverage.ini"
    if [ -d "$ini_dir" ] && [ ! -f "$ini_file" ]; then
      $SUDO tee "$ini_file" > /dev/null <<'INI'
; Auto-generated by setup_php.sh --dev
; Sets Xdebug to coverage-only mode so it doesn't slow down normal CLI runs.
zend_extension=xdebug
xdebug.mode=coverage
INI
      ok "Xdebug ini written to ${ini_file} (mode=coverage)"
    fi
  else
