...
<form method="post" class="validate" action="<?php print site_url('users/user_action:login'); ?>/back_to:<?php print base64_encode(getCurentURL()); ?>" id="logIn">
  ...
  <label class="clear left">
    <input type="checkbox" name="remember" value="y" class="left" style="margin-right: 7px" />
    Remember me</label>
  <a href="#" class="btn submit">Log in</a>
  <br class="clear" />
</form>
<div class="not-a-member"> <a href="<?php print site_url('users/user_action:login'); ?>" class="login">Join free</a> <a href="<?php print site_url('users/user_action:register'); ?>"><strong>Not a member?</strong></a> <a href="<?php print site_url('users/user_action:register'); ?>">Join now <em>It's fast and free</em></a> </div>
<?php else: ?>
<?php //p($post);
  ...
  <form method="post" action="#" id="comments_form" class="validate xform">
    ...
    <div style="width: 224px;">
     ...
     <?php $user_data = $this->users_model->getUserById ( $user_session ['user_id'] );   ?>
      ...
      <?php $thumb = $this->users_model->getUserThumbnail( $user_data['id'], 90); ?>
        <?php if($thumb != ''): ?>
