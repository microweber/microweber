  ...
  <tr>
    <td width="280px" style="padding-left: 74px"><div class="bar"> 
     ...
     .treeparam_object{
        ...
        <img src="<?php print_the_static_files_url() ; ?>icons/mouse.png"  border="0" alt="Right click for actions" align="left" /> Right click on the categories tree to add content and edit categories. <br />
        <br />
...
for ($x = 1; $x <= $number; $x++) : ?>
            ...
            <div class="d" style="display:none">
              <?php if($this->core_model->plugins_isPluginLoaded('rss_grabber') == true) : ?>
              <?php $grabber = new rss_grabber_model();
   $feeds = $grabber->getActiveFeeds();
   //var_dump($search_by_is_from_rss );
   ?>
              <?php if(!empty( $feeds )) : ?>
              <label class="ico"><img src="<?php print_the_static_files_url() ; ?>/icons/feed__arrow.png" alt=" " border="0">From RSS?</label>
          ...
          <table class="tablesorter tables" id="sortedtable" cellspacing="0" cellpadding="0">
            ...
            <tbody id="the_content_items_container">
              ...
              <tr id="content_row_id_<?php print $item['id']; ?>"> 
                <!-- ID -->
                <td style="display: none"><?php print $item['id']; ?><a href="<?php print $this->content_model->contentGetHrefForPostId($item['id']) ; ?>" target="_blank" ><img src="<?php print_the_static_files_url() ; ?>admin/icons/silk/page_go.png"  border="0" alt=" " /></a> <br />
                  <?php print $item['id']; ?>
                  <input type="checkbox" value="<?php print $item['id'] ?>" name="content_item_batch[]" /></td>
                <!-- Image -->
                <td style="display: none"><div class="cont_image" style="background-image: url('<?php print   $thumb = $this->content_model->contentGetThumbnailForContentId($item['id'], 110); ?>')"></div></td>
                ...
                <td style="display: none"><?php $temp= $this->comments_model->commentsGetForContentId( $item['id']); print (count(  $temp )); ?></td>
