...
<form action="<?php print site_url($this->uri->uri_string()); ?>" class="pull" method="post" enctype="multipart/form-data">
  ...
  <?php //var_dump( $form_values['custom_fields']['event_start']); ?> 
...
select_blog_category_for_section_id()
	...
	<?php foreach($sections as $section) : ?>
mySections[<?php print $section['id']  ?>]="<?php print $section['content_subtype_value']  ?>";
<?php endforeach;  ?>
	...
	do_the_taxonomy_categories_show_selected_values()
            ...
            <select name="content_parent" id="content_parent_items" onchange="select_blog_category_for_section_id()" >
              ...
              <?php foreach($sections as $section) : ?>
              <option onchange="select_blog_category_for_section_id()" value="<?php print $section['id'] ?>"  <?php if($form_values['content_parent'] == $section['id']) : ?>   selected="selected"  <?php endif; ?>><?php print site_url($section['content_url']); ?></option>
              <?php endforeach;  ?>
            </select>
            /</td>
...
function do_the_taxonomy_categories_show_selected_values(){
		...
		var find_c = tax_parent.find(":checked");
			...
			$.post("<?php print site_url('main/ajax_helpers_taxonomy_get_taxonomy_values_for_taxonomy_serialized_id_and_return_string') ?>", {  data_to_work: temp },
  function(data){
    ...
    <div id="tab_custom">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          ...
          <td><textarea name="custom_field_<?php print $i ?>"  rows="5" cols="50"><?php print $form_values['custom_field_'. $i]; ?></textarea></td>
        </tr>
        <?php endfor ; ?>
      </table>
    </div>
