...
(function($,window,undefined){
  ...
  fake_onhashchange = (function(){
    ...
    (function($) {
    	$.fn.hashtabs = function(forWho) {

    	    $(window).bind("hashchange load", function(){
                var hash = window.location.hash;
                if($("a[href='" + hash + "']").length>0 && $(hash + "-tab").length>0){
                   var hash = window.location.hash;
                   if($(".navMouseDown").length<1){
                      $('.nav a').removeClass('current');
                      $("a[href='" + hash + "']").addClass('current');
                      var scroller_left = parseFloat($('#form-manager-holder').css('left'));
                      var tab_left = $(hash + '-tab').offset().left - $('#form-manager').offset().left;
                      $("#form-manager-holder").not(":animated").animate({"left":scroller_left-tab_left});
                   }
                }
    	    });
            $(this).each(function(){
              ...
              $(this).find("a").each(function(){
                  ...
                  //$(href).attr('id', $(href).attr('id') + '-tab');
                  $(this).hover(function(){
                        $(this).addClass('navMouseDown')
                  }, function(){
                        $(this).removeClass('navMouseDown')
                  });
                  $(this).click(function(){
                      /*$(forWho).hide();
                      $(href + '-tab').show();*/
                      ...
                      var scroller_left = parseFloat($('#form-manager-holder').css('left'));
                      var tab_left = $(href + '-tab').offset().left - $('#form-manager').offset().left;
                      $("#form-manager-holder").not(":animated").animate({"left":scroller_left-tab_left});
                  });
              })
            });
...
function hideHelp(){
  ...
  if($('#modalHelp').length>0){
    $('#modalHelp').empty();
    $('#modalHelp').not(':animated').animate({
        "right":window_width-helpLeft+7,
        "width":0,
        "height":0,
        "opacity":0
    },'slow', function(){
...
$(document).ready(function(){
      $(".help").click(function(){
        var helpTop = $(this).offset().top;
        var helpLeft = $(this).offset().left;
        var window_width = $(window).width();
        if($(this).hasClass('active')){
            hideHelp();
        }
        else{
            var modalHelp = document.createElement('help');
            modalHelp.className = 'modalHelp';
            modalHelp.id = 'modalHelp';
            modalHelp.style.height = '0px';
            modalHelp.style.width = '0px';
            modalHelp.style.right = window_width-helpLeft+7+'px';
            ...
            $(modalHelp).not(':animated').animate({
              ...
              $('#helper').clone(true).appendTo(modalHelp);
              $(modalHelp).find('#helper').show();
            });
