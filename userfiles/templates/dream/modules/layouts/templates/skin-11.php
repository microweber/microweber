<?php

/*

type: layout

name: Icons with Text in Blue background

position: 11

*/

?>

<?php include 'settings_padding_front.php'; ?>

<section class="bg--primary safe-mode nodrop edit <?php print $padding ?>" field="layout-skin-11-<?php print $params['id'] ?>" rel="module">
    <div class="container">
        <div class="row">
            <div class="col-sm-4 cloneable">
                <div class="feature feature-1 allow-drop">
                    <i class="icon icon-Ship safe-element"></i>
                    <h6>New Startups</h6>
                    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500.</p>
                </div>
            </div>
            <div class="col-sm-4 cloneable">
                <div class="feature feature-1 allow-drop">
                    <i class="icon icon-Fingerprint-2 safe-element"></i>
                    <h6>Personal Portfolios</h6>
                    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500.</p>
                </div>
            </div>
            <div class="col-sm-4 cloneable">
                <div class="feature feature-1 allow-drop">
                    <i class="icon icon-Shop-2 safe-element"></i>
                    <h6>Online Storefronts</h6>
                    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500.</p>
                </div>
            </div>
        </div>
    </div>
</section>