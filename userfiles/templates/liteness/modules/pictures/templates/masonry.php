...
<?php if (is_array($data)): ?>
    ...
    <div class="mw-images-template-masonry mw-images-template-masonry-liteness" id="mw-gallery-<?php print $rand; ?>"
         style="position: relative;width: 100%;">
        <?php $count = -1; foreach ($data as $item): ?>
            <?php $count++; ?>
            <div class="masonry-item"
                 onclick="mw.tools.gallery.init(gallery<?php print $rand; ?>, <?php print $count; ?>)">
                <img src="<?php print thumbnail($item['filename'], 350); ?>" width="100%"/>
                <?php if ($item['title'] != '') { ?>
