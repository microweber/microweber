...
<div class="box-container">
    ...
    <?php if($have_social_login){ ?></ul></div> <?php } ?>
     <?php
        $ref = false;
        if(isset($_SERVER['HTTP_REFERER']) and strstr(site_url(), $_SERVER['HTTP_REFERER']) == 0){
            $ref = $_SERVER['HTTP_REFERER'];
        }
     ?>
    <div class="mw-ui-row mw-ui-row-drop-on-1024 sign-in">
        <div class="mw-ui-col">
            <div class="mw-ui-col-container"><div class="item-box"><module type="users/login" <?php if($ref != false){ print  'return="'.$ref.'"'; } ?> /></div></div>
        </div>
        <div class="mw-ui-col">
             <div class="mw-ui-col-container"><div class="item-box"><module type="users/register" <?php if($ref != false){ print  'return="'.$ref.'"'; } ?> /></div></div>
        </div>
    </div>
</div>
</div>