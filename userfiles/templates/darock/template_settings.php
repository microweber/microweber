<div id="settings-holder">
  ...
  <?php
    ...
    $color_scheme = get_option('color-scheme', 'mw-template-rock');
    if($color_scheme == ''){
      $color_scheme = 'color-white';
    }

    $font = get_option('font', 'mw-template-rock');

    if($font == ''){
      $font = 'lato';
    }

    $bgimage = get_option('bgimage', 'mw-template-rock');
    $custom_css_json = get_option('custom_css_json', 'mw-template-rock');

    $custom_bg =  get_option('custom_bg', 'mw-template-rock');
    $custom_bg_position = get_option('custom_bg_position', 'mw-template-rock');
    $custom_bg_size     = get_option('custom_bg_size', 'mw-template-rock');
        ...
        ***********************************************************************/
        ...
        $(uploader).bind('FileUploaded', function(a,b){
          ...
          mw.$("#image-upload-progress .mw-ui-progress-bar").width(0);
            ...
            parent.mw.$('body').css("backgroundImage", "url(" + b.src + ")");
        });
  ...
  <span class="mw-dropdown-value mw-ui-btn mw-dropdown-val">
    ...
    <div class="mw-dropdown-content">
      <ul>
        <li value="font-titilium" ><a style="font-family: 'Titillium Web'" href="#">Titillium Web</a></li>
...
<div>
  ...
  <a href="javascript:;" class="pick-scheme" style="background-color: #e4ad67" data-value='orange'></a>
  <a href="javascript:;" class="pick-scheme" style="background-color: #BC83CC" data-value='purple'></a>
  <a href="javascript:;" class="pick-scheme" style="background-color: #ff1a60" data-value='magenta'></a>
...
<div class="body-bgs-holder">
    ...
    <span id="mwbgimages"><a href="javascript:;" class="pick-mwimage scheme-transparent" data-value='none'></a></span>
<script src="//api.microweber.com/mwtpls/"></script>
<script>
$(window).bind('load', function(){

   $(mw.TemplateImages).each(function(index){
     if(index < 9){
        $("#mwbgimages").append('<a href="javascript:;" class="pick-mwimage" data-value="'+this.url+'" style="background-image: url('+this.url+')"></a>');
     }
     else if(index == 9){
       $("#mwbgimages").append('<span class="mw-ui-btn-invert" id="showmoreimg">More</span>');
       return false;
     }
   });

   $(".pick-mwimage").bind("click", function(){
        mw.$('#custom_bg').val($(this).dataset('value')).trigger('change');
        parent.mw.$('body').css("backgroundImage", "url(" + $(this).dataset('value') + ")");
   });
   $("#showmoreimg").bind('click', function(){
       var html = '';
       $(mw.TemplateImages).each(function(){
            html += '<a href="javascript:;" class="pick-mwimage" data-value="'+this.url+'" style="background-image: url('+this.url+')"></a>';
       });
       html += '<hr><div class="text-center"><span class="mw-ui-btn mw-ui-btn-invert">Done</span></div>';
       var modal = window.top.mw.modal({content:html, name:'alltplimages', title:'Select ackground image', width:445});
       $(".pick-mwimage", modal.container).bind("click", function(){
            mw.$('#custom_bg').val($(this).dataset('value')).trigger('change');
            parent.mw.$('body').css("backgroundImage", "url(" + $(this).dataset('value') + ")");
       });
   });
...
<div id="background-options" style="display:block;">
...
<input type="hidden" class="mw_option_field" id="color-scheme-input" name="color-scheme" data-option-group="mw-template-rock"  />
<input type="hidden" class="mw_option_field" id="font-input" name="font" data-option-group="mw-template-rock"  />
<input type="hidden" class="mw_option_field" id="bgimage" name="bgimage" data-option-group="mw-template-rock"  />
<input type="hidden" class="mw_option_field" id="custom_css_json" name="custom_css_json" data-option-group="mw-template-rock"  />

<input type="hidden" class="mw_option_field" id="custom_bg" name="custom_bg" data-option-group="mw-template-rock"  />

<input type="hidden" class="mw_option_field" id="custom_bg_position" name="custom_bg_position" data-option-group="mw-template-rock"  />
<input type="hidden" class="mw_option_field" id="custom_bg_size" name="custom_bg_size" data-option-group="mw-template-rock"  />


<input type="hidden" class="mw_option_field" id="kuler_colors" name="kuler_colors" data-option-group="mw-template-rock"  />
