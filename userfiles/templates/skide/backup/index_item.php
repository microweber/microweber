<?php // p($log); ?>
<?php $author = CI::model('users')->getUserById($log['user_id']); ?>
		<?php $thumb = CI::model('users')->getUserThumbnail( $log['user_id'], 45); ?>
  	<?php $data = CI::model('core')->getById($log['to_table'],$log['to_table_id']); ?>	
 <?php
 $addtinal_style = '';
  if(($log['to_table'] == 'table_votes') and ($data['to_table'] == 'table_followers')){
  $addtinal_style = "display:none;";
 CI::model('users')->logDeleteById($log['id']);
  }
...
<div class="users-status" style="<?php print $addtinal_style ?>">
	      ...
	      <div class="users-status-content">
              ...
              <?php if($log['user_id'] != CI::model('users')->userId()): ?>
...
<?php case 'table_users_statuses': ?>
 <?php $statusRow = CI::model('users')->statusGetById ($log['to_table_id']);   ?>
...
<?php $content_data = CI::model('content')->contentGetByIdAndCache ( $log ['to_table_id'] );
$url = CI::model('content')->getContentURLByIdAndCache($log ['to_table_id']);
 ?>
...
<?php case 'table_votes': ?>
 ...
 <?php $content_data = CI::model('content')->contentGetByIdAndCache ( $data ['to_table_id'] );
$url = CI::model('content')->getContentURLByIdAndCache($data ['to_table_id']);
...
<?php $data = CI::model('core')->getById($log['to_table'],$log['to_table_id']);
$content_data = CI::model('content')->contentGetByIdAndCache ( $data ['id'] );
$url = CI::model('content')->getContentURLByIdAndCache($data ['id']);
 ?>
 <?php // p($data); ?>
...
<?php endif;?>
<a href="<?php print site_url('userbase/action:profile/username:'.$data2['username']) ?>"><?php print CI::model('users')->getPrintableName( $data['follower_id']) ?></a>
<?php else : ?>
<?php CI::model('users')->logDeleteById($log['id']); ?>
<?php endif;?>
...
<?php endswitch;?>
	          ...
	          <div class="status-nav">
	              <ul>
	                ...
	                <li><a href="#" class="voteUp dashboard-votes-<?php print $log['id']; ?>"><?php print CI::model('votes')->votesGetCount($log['to_table'], $log['to_table_id'], '1 year'); ?></a></li>
                      <li><a href="javascript:mw.content.Vote('<?php print CI::model('core')->securityEncryptString($log['to_table']) ?>', '<?php print CI::model('core')->securityEncryptString($log['to_table_id']) ?>', '.dashboard-votes-<?php print $log['id'] ?>');" class="like">like</a></li>
                    
                    
                    
	                <li><a href="javascript:mw.comments.getComments('<?php print $log['to_table'] ?>', '<?php print $log['to_table_id'] ?>', '#comments-for-dashboard-ajax-<?php print $log['id'] ?>');"   class="cmm" id="status-comments-<?php print $log['id'] ?>"><?php print CI::model('content')->commentsGetCount($log['to_table'], $log['to_table_id'], $is_moderated = false); ?></a></li>
	      ...
	      <div class="status-comments" id="users-statuses-comments-list-<?php print $log['id'] ?>">
	          <div class="status-comment add-comment">
	              ...
	              <form action="#" method="post" class="commentForm" onsubmit="setTimeout(function(){mw.comments.getComments('<?php print $log['to_table'] ?>', '<?php print $log['to_table_id'] ?>', '#comments-for-dashboard-ajax-<?php print $log['id'] ?>')}, 1500);">
	              	...
	              	<input type="hidden" name="to_table" value="<?php print CI::model('core')->securityEncryptString($log['to_table']); ?>">
	              	<input type="hidden" name="to_table_id" value="<?php print CI::model('core')->securityEncryptString($log['to_table_id']); ?>">
