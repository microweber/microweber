...
<?php $params = array();
 ...
 $params['type'] = 'remove';
 //$params['type'] = 'inherit';
 $url = CI::model('core')->urlConstruct($base_url = site_url('userbase'),$params);
...
<div class="main">
  ...
  <div class="post" style="padding-left:0"> <a href="<?php print site_url('userbase/action:profile/username:').$item['username'] ?>" class="eimg"> <span style="background-image: url('<?php print   $thumb = CI::model('users')->getUserThumbnail($item['id'], 80); ?>');"></span> </a>
    <h2 class="post-title"><a href="<?php print site_url('userbase/action:profile/username:').$item['username'] ?>"><?php print CI::model('users')->getPrintableName (  $item['id'], 'full' ); ?></a></h2>
