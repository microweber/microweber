<div class="col-<?php echo $settings['field_size']; ?>">
    <div class="form-group">
        ...
        <?php if($settings['show_label']): ?>
            <label class="control-label me-2 align-self-center mb-3">
                <?php echo $data['name']; ?>
        ...
        <?php if ($settings['as_text_area']): ?>
            <textarea type="text" class="form-control" rows="<?php echo $settings['rows']; ?>" cols="<?php echo $settings['cols']; ?>" <?php if ($settings['required']): ?>required="true"<?php endif; ?> data-custom-field-id="<?php echo $data['id']; ?>" data-custom-field-error-text="<?php echo $data['error_text']; ?>" name="<?php echo $data['name']; ?>" placeholder="<?php echo $data['placeholder']; ?>"><?php echo $data['value']; ?></textarea>
        <?php else: ?>
