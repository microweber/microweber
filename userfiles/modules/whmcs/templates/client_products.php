...
<div> <a class="btn btn-link right"  href="<?php   print($goto); ?>">Add new website</a> </div>
<?php $user = get_user(); ?>
<?php if(isset($_COOKIE['mw_is_veririfed_valid_email']) and $_COOKIE['mw_is_veririfed_valid_email'] == 'n' and isset($user['email']) and (filter_var($user['email'], FILTER_VALIDATE_EMAIL))):  ?>
<div class="alert alert-warning" role="alert" style="clear:both"> 
  <script>

resend_conf_mail = function(){
	 
	   $.ajax({
		  url: "<?php print api_url('mw_resend_cofirmation_email') ?>" 
		}).done(function() {
  ...
  <?php 
			   ...
			   $wl = new WebmailLinker();
				...
				if($provider = $wl->getProviderByEmailAddress($user['email'])) {
				  $html = '<a href="%s">Please confirm your email at %s</a>';
				  printf($html, htmlspecialchars($provider['url']), htmlspecialchars($provider['name']));
				} else {
					 $html = 'Please confirm your email address %s';
				  printf($html, htmlspecialchars($user['email']));
				}
			   
			    ?>
  <a type="button" class="btn btn-primary mw_resend_cofirmation_email" href="javascript:resend_conf_mail();" >Resend</a> </div>
<?php endif; ?>
<?php
...
<table class="user-sites-table user-sites-table-small">
  ...
  <tbody>
    ...
    <?php $isHosting = ( isset($item['groupname']) and strtolower($item['groupname']) == 'hosting' ) == true;  ?>
    <?php if(!$isTerminated and !$isCancelled  and $isHosting): ?>
    <tr >
    ...
    ?>
      <td><?php if($isActive): ?>
        ...
        <?php endif; ?></td>
      <td><span class="label <?php print $cls; ?>" title="Your site is <?php print($item['status']); ?>.">
  ...
  $cookie_site_pending = $item['id'];
   if(!isset($_COOKIE['cookie_site_'.$cookie_site_pending])){
	  
	  
	  setcookie('cookie_site_'.$cookie_site_pending,$item['regdate']);
 ...
 $( document ).ready(function() {
   refresh_this_module();
        ...
        <?php else: ?>
        <a href="javascript:mw.reload_module('#<?php print $params['id'] ?>')" title="Click here to refresh the status">
        <?php  print($item['status']); ?>
        </a>
        <?php endif; ?>
        <?php else: ?>
 ...
 <td><span title="You are using <?php print($item['name']); ?>."><?php print($item['name']); ?></span></td>
    <a href="<?php print $goto ?>" class="user-sites-table-btn">Upgrade</a> </td>-->
      ...
      <td <?php if(!$isActive){ print " colspan='2'"; } ?>><?php
       ...
       ?>
        ...
        <?php
                ...
                $goto = api_url('panel_user_link').'?goto='.urlencode($goto);
            ?>
        <a class="blue" href="<?php print $goto ?>" target="_blank"><?php print $item['name']; ?></a>
        <?php endif; ?></td>
    </tr>
    <?php endif; ?>
    <?php endforeach; ?>
