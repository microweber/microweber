...
<li <? print $addtinal_style ?> id="log_item_<? print $log_check['id'] ?>"> <a href="<?php print user_link($author['id']) ?>" class="user_photo" style="background-image: url('<?php print $thumb; ?>')"></a> <a href="<?php print user_link($author['id']) ?>" class="mw_blue_link"><? print user_name($author['id']); ?></a> <? print $action_data['msg']; ?>
  <div class="user_activity_bar">
    ...
    <div>
      ...
      <a  class="user_activity_likes right"  href="<? print voting_link($log_check['to_table_id'], '#post-likes-'.$log_check['to_table_id'], $log_check['to_table']); ?>"><strong id="post-likes-<? print ($log_check['to_table_id']); ?>"><? print votes_count($log_check['to_table_id'], false,$log_check['to_table'] ); ?></strong> Love</a>
      <? endif; ?>
    </div>
  </div>
  <? if($action_data['allow_comments'] == true): ?>
  ...
  ?>
  <? comment_post_form($log_check['rel_table_id'],'layouts/dashboard/index_item_comments.php', $log_check['rel_table'])  ?>
  <div id="<? print $update_element ?>">
    <? comments_list($log_check['rel_table_id'], 'layouts/dashboard/index_item_comments_list.php', $log_check['rel_table'])  ?>
  </div>
  <? endif; ?>
</li>
<? endif; ?>
