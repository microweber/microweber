<?php use shop\shipping\gateways\standard\shipping_standard;

$rand = $module_wrapper_id = 'shipping_standard_' . md5($params['module']). md5($params['id']);

$data_disabled = mw('shop\shipping\gateways\standard\shipping_standard')->get("is_active=0");
$shipping_cost = mw('shop\shipping\gateways\standard\shipping_standard')->get_cost();
$shipping_cost = floatval($shipping_cost);
$countries_used = array();
$countries_all = array();

$data = mw('shop\shipping\gateways\standard\shipping_standard')->get_available_countries();
?>
    <script type="text/javascript">
        mw.require('forms.js', true);
    </script>
    <script type="text/javascript">
        mw_shipping_standard_last_val<?php print $rand; ?> = null;
        function mw_shipping_<?php print $rand; ?>(data) {

            if(mw_shipping_standard_last_val<?php print $rand; ?>  === data){
                return;
            }

            mw_shipping_standard_last_val<?php print $rand; ?> = data

            $.post("<?php print $config['module_api']; ?>/shipping_standard/set", data)
            .done(function (msg) {
