<?php use shop\shipping\gateways\firstclass\shipping_firstclass;

$rand = $module_wrapper_id = 'shipping_firstclass_' . md5($params['module']). md5($params['id']);

$data_disabled = mw('shop\shipping\gateways\firstclass\shipping_firstclass')->get("is_active=0");
$shipping_cost = mw('shop\shipping\gateways\firstclass\shipping_firstclass')->get_cost();
$shipping_cost = floatval($shipping_cost);
$countries_used = array();
$countries_all = array();

$data = mw('shop\shipping\gateways\firstclass\shipping_firstclass')->get_available_countries();
?>
    <script type="text/javascript">
        mw.require('forms.js', true);
    </script>
    <script type="text/javascript">
        mw_shipping_firstclass_last_val<?php print $rand; ?> = null;
        function mw_shipping_<?php print $rand; ?>(data) {

            if(mw_shipping_firstclass_last_val<?php print $rand; ?>  === data){
                return;
            }

            mw_shipping_firstclass_last_val<?php print $rand; ?> = data

            $.post("<?php print $config['module_api']; ?>/shipping_firstclass/set", data)
            .done(function (msg) {
