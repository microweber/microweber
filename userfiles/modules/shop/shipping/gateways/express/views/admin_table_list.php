<script>
    function mw_admin_edit_country_item_popup(id) {
        ...
        // }
        //
        // mw.load_module('shop/shipping/gateways/express/add_country', '#mw_admin_edit_country_item_module', null, params);


        var modal_id = 'mw_admin_edit_country_item_popup_modal_opened';
        var dialog = mw.top().dialogIframe({
             url: route('live_edit.module_settings')+'?type=shop/shipping/gateways/express/add_country&live_edit=true&id=mw_admin_edit_country_item_popup_modal_opened__module&edit_id='+id+'&from_url=<?php print site_url() ?>',
            title:modalTitle,
            id: modal_id,
    ...
    mw.shipping_country = {
        delete_country : function(id){
            ...
            mw.tools.confirm(q, function(){
                var obj = {};
                obj.id = id;
                $.post("<?php print $config['module_api']; ?>/shipping_express/delete",  obj, function(data){
                    mw.$(".country-id-"+id).fadeOut();
                    mw.reload_module_everywhere('[data-parent-module="shop/shipping"]');
                    mw.reload_module_everywhere('shop/shipping/gateways/express/admin');
                    if(window.parent != undefined && window.parent.mw != undefined){
                        mw.reload_module_everywhere('shop/shipping/gateways/express');
                    }
                });
            });
        },
...
<div class="">
    ...
    <div class="mw-shipping-items shipping_express_holder table-responsive mt-3" id="shipping_express_holder<?php if ($active_or_disabled == 'active'): ?>_active<?php endif; ?>">
