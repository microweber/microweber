...
<?php if ($cart_totals): ?>
    <div class="row ml-auto mb-2">
        <h5 class="mb-2"><?php _e("Total amount"); ?></h5>
        <table class="table">
            <?php foreach ($cart_totals as $cart_total_key => $cart_total): ?>
                <?php if ($cart_total_key != 'total' and $cart_total and is_array($cart_total) and !empty($cart_total) and isset($cart_total['value']) and $cart_total['value']): ?>
                    ...
                    <thead>
                    <tr>
                        <th class="pl-0"><?php _e($cart_total['label']); ?>: <?php print currency_format($cart_total['value']); ?></th>
                    </tr>
                    </thead>
                <?php endif; ?>
            <?php endforeach; ?>
...
<?php if ($cart_totals): ?>
    <div class="row ml-auto">
        ...
        <h5 class="col-xs-6 checkout-modal-total-label mr-1 font-weight-bold mb-2"><?php _e("Total"); ?>:</h5>
