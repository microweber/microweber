...
<form class="add_content_form form" method="post" id="<? print $form_id ?>" action="#" enctype="multipart/form-data">
  ...
  <? $cats = $this->taxonomy_model->getCategoriesForContent($the_post['id'], true); ?>
  <?
  ...
  $master_cats = $this->taxonomy_model->getParents($category);
 ...
 $subcats = $this->taxonomy_model->getChildrensRecursiveAndCache($master_category, $type = 'category', $visible_on_frontend = false);
 if(empty($subcats)){
	 $subcats = $this->taxonomy_model->getChildrensRecursiveAndCache($category, $type = 'category', $visible_on_frontend = false);
 }
  ?>
