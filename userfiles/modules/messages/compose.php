...
<form method="post" id="message-compose" class="form">
  ...
  <input name="mk" type="hidden" value="<?php print (CI::model('core')->securityEncryptString ( user_id() )) ?>" />
...
<? if(!$conversation): ?>
  ...
  <span class="linput">
  <input name="subject" type="text" style="width:370px" class="required" />
  </span>
   <? endif; ?>
  ...
  <a href="javascript:;" onClick="if($(this).parents('form').isValid()){mw.users.UserMessage.sendQuick(this)}" class="btn right">Send</a>
</form>
