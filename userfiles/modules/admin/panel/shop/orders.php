...
<div id="mw-order-table-holder">
  ...
  <table class="mw-ui-table mw-order-table mw-ui-table-spacious" id="shop-orders" cellpadding="0" cellspacing="0" width="100%">
    <colgroup>
    <col>
    <col>
    <col>
    ...
    <tbody>
      ...
      <tr class="mw-order-item mw-order-item-<?php print $item['id'] ?> mw-order-status-<?php print $item['order_status'] ?> tip" data-showon="#vieorder-<?php print $item['id']; ?>"  data-tipposition="top-center" data-tipcircle="true" data-tip="#product-tip-<?php print $item['id'] ?>">
        ...
        <div id="product-tip-<?php print $item['id'] ?>" style="display: none"> <span class="product-thumbnail-tooltip" style="background-image: url(<?php print thumbnail($p, 120, 120); ?>)"></span> </div>
        <?php endif; ?>
        <?php } ?>
        ...
        <td class="mw-order-item-status"><?php  if($item['order_status'] == false): ?>
          ...
          <?php else : ?>
          <span class="mw-order-item-status-pending">
          <?php _e("Pending"); ?>
          </span>
          <?php endif; ?></td>
        ...
        <td class="mw-order-item-edit" width="90" align="center"><span class="mw-icon-close show-on-hover tip" data-tip="<?php _e("Delete"); ?>" data-tipposition="top-center"  onclick="mw_delete_shop_order('<?php print ($item['id']) ?>');"></span> <a class="show-on-hover mw-ui-btn-invert mw-ui-btn mw-ui-btn-small" id="vieorder-<?php print $item['id']; ?>" href="#vieworder=<?php print ($item['id']) ?>">
          <?php _e("View order"); ?>
          </a></td>
      </tr>
      <?php endforeach; ?>
  ...
  <label class="mw-ui-label">
    <?php _e("Abandoned Carts Section helps you analyze why some customers aren't checking out."); ?>
  </label>
  ...
  <?php foreach ($orders as $item) : ?>
  <h2>
    <?php _e("Abandoned Cart ID"); ?>
    : <?php print $item['id']; ?></h2>
    ...
    <tbody>
      <tr class="mw-order-item-<?php print $item['id'] ?> no-hover" >
        <td><?php $cart_items = get_cart('order_completed=0&session_id='.$item['session_id']); ?>
          ...
          <?php endif;?></td>
        <td style="padding: 20px;"><label class="mw-ui-label pull-right"> <span class="mw-icon-lite-clock-outline" style="font-size: 16px;top:-1px;right:2px;"></span> <span class="mw-ui-label-small tip" data-tipposition="top-center" data-tip="<?php _e("Last activity on"); ?>: <?php print $item['updated_at'] ?>"><?php print mw('format')->ago($item['updated_at']); ?></span> </label>
          <style scoped="scoped">
          ...
          <div class="mw-ui-field-holder" style="padding-bottom: 20px;">
            <label class="mw-ui-label mw-ui-label-inline" style="width: 120px;">
              <?php _e("Recover URL"); ?>
          ...
          <div class="mw-ui-btn-nav pull-right"> <a class="mw-ui-btn" href="javascript:mw_delete_shop_order('<?php print ($item['session_id']) ?>',1);">
            <?php _e("Delete cart"); ?>
