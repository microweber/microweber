...
if ($_POST) {
	...
	if ($_POST ['step1']) {
		...
		if (! empty ( $q )) {
			...
			foreach ( $q as $q1 ) {
				...
				$q_check = $this->core_model->getById ( $q1 ['to_table'], $id = $q1 ['to_table_id'] );
				if (! empty ( $q_check )) {
					//	print "CF found {$q1 ['to_table']} - {$q1 ['to_table_id']} \n\r <br>";
				} else {
					print "Custom field NOT FOUND! {$q1 ['to_table']} - {$q1 ['to_table_id']} \n\r <br>";
					
					$clean = $this->core_model->deleteDataById ( $table_custom_fields, $q1 ['id'] );
		...
		if (! empty ( $q )) {
			...
			foreach ( $q as $q1 ) {
				...
				$q_check = $this->core_model->getById ( $table_taxonomy, $id = $q1 ['parent_id'] );
				if (! empty ( $q_check )) {
					//print "table_taxonomy_items found {$q1 ['to_table']} - {$q1 ['to_table_id']} \n\r <br>";
				} else {
	...
	print '</pre>';
	$clean = $this->core_model->cacheDeleteAll ();
	//var_dump ( $k );
} else {
