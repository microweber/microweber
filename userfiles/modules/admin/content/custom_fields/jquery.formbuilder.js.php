...
(function ($) {
			...
			var controlBox = function (target) {
				...
				/*	select += '<option value="textarea">' + opts.messages.paragraph + '</option>';
                   ...
                   $('#' + frmb_id).find('input,select').die( "change" )
                     ...
                     $('#' + frmb_id).find('input,select,textarea').die( "blur" )
                     $('#' + frmb_id).find('input,select,textarea').live("blur", function(){ 
                      save();
                    });   
	...
	li += '<a id="del_' + last_id + '" class="del-button delete-confirm" rel="' + f_obj.id + '" name="' + f_obj.custom_field_name.replace(/_/gi, " ") + '" href="#" title="' + opts.messages.remove_message + '"><span>' + opts.messages.remove + '</span></a>';
					} else {
...
//$("#select_cf_for-" + last_id).find(f1).attr("selected", "selected");
  ...
  select_cf_for1 = '<option selected="selected" value="' + f_obj.field_for + '">' + f_obj.field_for.replace(/_/gi, " ") + '</option>';
  //alert(  select_cf_for1);
  $(select_cf_for1).prependTo("#select_cf_for-" + last_id);
  
  
  //$("#select_cf_for-" + last_id).append(select_cf_for1); 
			...
			$('.delete-confirm').live('click', function () {
                ...
                var delete_id1 = $(this).attr("rel").replace(/del_/, '');
                  var delete_id2 = $(this).attr("name").replace(/cf_mw_del_/, '');
				if (confirm($(this).attr('title'))) {
					$('#frm-' + delete_id + '-item').animate({
						...
						$.ajax({
							type: "POST",
							url: opts.del_url+delete_id1+'/name:'+delete_id2,
							success: function () {
