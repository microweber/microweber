...
function get_dashboard_action($log_id) {
	...
	switch ($data ['rel']) {
		...
		case 'table_votes' :
			...
			if ($vote ['rel'] == 'content') {
				$more = get_instance()->core_model->getCustomFields ( 'content', $vote ['rel_id'] );
		...
		case 'content' :
			...
			if (empty ( $content_data )) {
				...
				$more = get_instance()->core_model->getCustomFields ( 'content', $content_data ['id'] );
